...
**PDCA CLI** is the command-line interface that bootstraps projects with the PDCA Kit framework. It sets up the necessary directory structures, templates, and AI agent integrations to support the Spec-Driven Development workflow.
...
Each AI agent is a self-contained **integration subpackage** under `src/pdca_cli/integrations/<key>/`. The subpackage exposes a single class that declares all metadata and inherits setup/teardown logic from a base class. Built-in integrations are then instantiated and added to the global `INTEGRATION_REGISTRY` by `src/pdca_cli/integrations/__init__.py` via `_register_builtins()`.

```
src/pdca_cli/integrations/
...
| Skill directories (`pdca-<name>/SKILL.md`) | `SkillsIntegration` |
...
In `src/pdca_cli/integrations/__init__.py`, add one import and one `_register()` call inside `_register_builtins()`. Both lists are alphabetical:
...
Set `context_file` on the integration class. The base integration setup creates or updates the managed PDCA Kit section in that file, and uninstall removes the managed section when appropriate.

The managed section is owned by the bundled `agent-context` extension (`extensions/agent-context/`). All configuration flows through the extension's own config file at `.pdca/extensions/agent-context/agent-context-config.yml`:
...
pdca init my-project --integration <key>
...
|---|---|---|
| `command_filename(template_name)` | Custom file naming or extension | Copilot → `pdca.{name}.agent.md` |
| `options()` | Integration-specific CLI flags via `--integration-options` | Codex → `--skills` flag, Copilot → `--skills` flag |
| `setup()` | Custom install logic (companion files, settings merge) | Copilot → `.agent.md` + `.prompt.md` + `.vscode/settings.json` (default) or `pdca-<name>/SKILL.md` (skills mode) |
| `teardown()` | Custom uninstall logic | Rarely needed; base handles manifest-tracked files |

**Example — Copilot (fully custom `setup`):**

Copilot extends `IntegrationBase` directly because it creates `.agent.md` commands, companion `.prompt.md` files, and merges `.vscode/settings.json`. It also supports a `--skills` mode that scaffolds `pdca-<name>/SKILL.md` under `.github/skills/` using composition with an internal `_CopilotSkillsHelper`. See `src/pdca_cli/integrations/copilot/__init__.py` for the full implementation.
...
- Commands are scaffolded as `pdca-<name>/SKILL.md` under `.github/skills/`
- No companion `.prompt.md` files are generated
- No `.vscode/settings.json` merge
- `post_process_skill_content()` injects a `mode: pdca.<stem>` frontmatter field
- `build_command_invocation()` returns `/pdca-<stem>` instead of bare args
...
# Skills mode: pdca-<name>/SKILL.md under .github/skills/
pdca init my-project --integration copilot --integration-options="--skills"
```
...
5. Sets `context_file = "AGENTS.md"` so the base setup manages the PDCA Kit context section there
...
2. **Forgetting context configuration**: The bundled `agent-context` extension reads from `.pdca/extensions/agent-context/agent-context-config.yml`. New integrations only need to set `context_file` on the class — markers and dispatcher scripts are managed centrally.
