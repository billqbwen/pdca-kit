...
```bash
pdca workflow run pdca -i spec="Build a kanban board with drag-and-drop task management" -i scope=full
```

> **Note:** All workflow commands require a project already initialized with `pdca init`.
...
```bash
pdca workflow remove <workflow_id>
```
...
```bash
pdca workflow info <workflow_id>
```
...
Adds a custom catalog URL to the project's `.pdca/workflow-catalogs.yml`.
...
1. **Environment variable** — `PDCA_WORKFLOW_CATALOG_URL` overrides all catalogs
2. **Project config** — `.pdca/workflow-catalogs.yml`
3. **User config** — `~/.pdca/workflow-catalogs.yml`
4. **Built-in defaults** — official catalog + community catalog

## Workflow Definition

Workflows are defined in YAML files. Here is the built-in **Full SDD Cycle** workflow that ships with PDCA Kit:
...
```bash
pdca workflow run pdca -i spec="Build a kanban board with drag-and-drop task management"
```
...
| Type         | Purpose                                          |
| ------------ | ------------------------------------------------ |
| `command`    | Invoke a PDCA Kit command (e.g., `pdca.plan`) |
...
Each workflow run persists its state at `.pdca/workflows/runs/<run_id>/`:

- `state.json` — current run state and step progress
- `inputs.json` — resolved input values
- `log.jsonl` — step-by-step execution log

This enables `pdca workflow resume` to continue from the exact step where a run was paused (e.g., at a gate) or failed.

## FAQ

### What happens when a workflow hits a gate step?

The workflow pauses and waits for human input. Run `pdca workflow resume <run_id>` after reviewing to continue.

### Can I run the same workflow multiple times?

Yes. Each run gets a unique ID and its own state directory. Use `pdca workflow status` to see all runs.

### Who maintains workflows?

Most workflows are independently created and maintained by their respective authors. The PDCA Kit maintainers do not review, audit, endorse, or support workflow code. Review a workflow's source before installing and use at your own discretion.
