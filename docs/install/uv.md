...
[uv](https://docs.astral.sh/uv/) is a fast Python package manager by [Astral](https://astral.sh/). PDCA Kit uses `uv` (via `uvx` or `uv tool install`) to run the `pdca` CLI without polluting your global Python environment.
