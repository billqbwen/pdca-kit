# Contributing to PDCA Kit

Hi there! We're thrilled that you'd like to contribute to PDCA Kit. Contributions to this project are [released](https://help.github.com/articles/github-terms-of-service/#6-contributions-under-repository-license) to the public under the [project's open source license](LICENSE).
...
4. **Run prerequisites first** — commands that depend on earlier commands (e.g., `/pdca.tasks` requires `/pdca.plan` which requires `/pdca.define`) must be run in order.
...
- src/pdca_cli/*.py → CLI commands (`pdca init`, `pdca check`, `pdca extension *`, `pdca preset *`); test the affected CLI command and, for init/scaffolding changes, at minimum test /pdca.define
...
- presets/*/* → test preset scaffolding via `pdca init` with the preset
- pyproject.toml → packaging/bundling; test `pdca init` and verify bundled assets
...
- T1: /pdca.command — (reason)
- T2: /pdca.command — (reason)
~~~

## AI contributions in PDCA Kit

> [!IMPORTANT]
>
> If you are using **any kind of AI assistance** to contribute to PDCA Kit,
> it must be disclosed in the pull request or issue.

We welcome and encourage the use of AI tools to help improve PDCA Kit! Many valuable contributions have been enhanced with AI assistance for code generation, issue detection, and feature definition.

That being said, if you are using any kind of AI assistance (e.g., agents, ChatGPT) while contributing to PDCA Kit,
...
- **Clear rationale** - You can explain why the change is needed and how it fits within PDCA Kit's goals
...
- Generic suggestions that don't address specific PDCA Kit needs
