...
if [[ ! -f "$IMPL_PLAN" ]]; then
    ...
    echo "Run __PDCA_COMMAND_PLAN__ first to create the implementation plan." >&2
    exit 1
fi
...
if [[ -z "$TASKS_TEMPLATE" ]] || [[ ! -f "$TASKS_TEMPLATE" ]]; then
    ...
    echo "Template 'tasks-template' was not found in any supported location (overrides, presets, extensions, or shared core). Add an override at .pdca/templates/overrides/tasks-template.md, or run 'specify init' / reinstall shared infra to restore the core .pdca/templates/tasks-template.md template." >&2
    exit 1
fi
