...
# Get repository root, prioritizing .pdca directory over git
...
get_current_branch() {
    # First check if PDCA_FEATURE environment variable is set
    if [[ -n "${PDCA_FEATURE:-}" ]]; then
        echo "$PDCA_FEATURE"
        return
    fi

    # Then check git if available at the pdca-kit root (not parent)
...
# Check if we have git available at the pdca-kit root level
...
pdca_kit_effective_branch_name() {
    local raw="$1"
    if [[ "$raw" =~ ^([^/]+)/([^/]+)$ ]]; then
        printf '%s\n' "${BASH_REMATCH[2]}"
    else
...
check_feature_branch() {
    ...
    local branch
    branch=$(pdca_kit_effective_branch_name "$raw")
...
# Safely read .pdca/feature.json's "feature_directory" value.
...
read_feature_json_feature_directory() {
    local repo_root="$1"
    local fj="$repo_root/.pdca/feature.json"
...
# Returns 0 when .pdca/feature.json lists feature_directory that exists as a directory
# and matches the resolved active FEATURE_DIR (so __PDCA_COMMAND_PLAN__ can skip git branch pattern checks).
...
find_feature_dir_by_prefix() {
    local repo_root="$1"
    local branch_name
    branch_name=$(pdca_kit_effective_branch_name "$2")
    local specs_dir="$repo_root/specs"
...
get_feature_paths() {
    ...
    if [[ -n "${PDCA_FEATURE_DIRECTORY:-}" ]]; then
        feature_dir="$PDCA_FEATURE_DIRECTORY"
        # Normalize relative paths to absolute under repo root
        [[ "$feature_dir" != /* ]] && feature_dir="$repo_root/$feature_dir"
    elif [[ -f "$repo_root/.pdca/feature.json" ]]; then
...
#   1. .pdca/templates/overrides/
#   2. .pdca/presets/<preset-id>/templates/ (sorted by priority from .registry)
#   3. .pdca/extensions/<ext-id>/templates/
#   4. .pdca/templates/ (core)
resolve_template() {
    local template_name="$1"
    local repo_root="$2"
    local base="$repo_root/.pdca/templates"
    ...
    local presets_dir="$repo_root/.pdca/presets"
    if [ -d "$presets_dir" ]; then
        ...
        if [ -f "$registry_file" ] && command -v python3 >/dev/null 2>&1; then
            ...
            if sorted_presets=$(PDCA_REGISTRY="$registry_file" python3 -c "
import json, sys, os
try:
    with open(os.environ['PDCA_REGISTRY']) as f:
        data = json.load(f)
...
" 2>/dev/null); then
    ...
    local ext_dir="$repo_root/.pdca/extensions"
    if [ -d "$ext_dir" ]; then
                ...
                if [ -n "$sorted_presets" ]; then
                    ...
                    while IFS= read -r preset_id; do
                        ...
                        if [ -f "$manifest" ] && command -v python3 >/dev/null 2>&1; then
                            ...
                            local py_stderr
                            py_stderr=$(mktemp)
                            result=$(PDCA_MANIFEST="$manifest" PDCA_TMPL="$template_name" python3 -c "
import sys, os
try:
    import yaml
except ImportError:
    ...
    sys.exit(0)
try:
    with open(os.environ['PDCA_MANIFEST']) as f:
        data = yaml.safe_load(f)
    for t in data.get('provides', {}).get('templates', []):
        if t.get('name') == os.environ['PDCA_TMPL'] and t.get('type', 'template') == 'template':
