...
# Use common.ps1 functions which prioritize .pdca over git
...
if (-not $DryRun) {
    ...
    # Set the PDCA_FEATURE environment variable for the current session
    $env:PDCA_FEATURE = $branchName
}
...
if ($Json) {
    ...
    if (-not $DryRun) {
        Write-Output "PDCA_FEATURE environment variable set to: $branchName"
    }
}
