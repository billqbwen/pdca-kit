...
function Find-PdcaRoot {
    ...
    while ($true) {
        if (Test-Path -LiteralPath (Join-Path $current ".pdca") -PathType Container) {
            return $current
        }
...
# True when .pdca/feature.json pins an existing feature directory that matches the
# active FEATURE_DIR from Get-FeaturePathsEnv (so __PDCA_COMMAND_PLAN__ can skip git branch pattern checks).
function Test-FeatureJsonMatchesFeatureDir {
    ...
    $featureJson = Join-Path (Join-Path $RepoRoot 'pdca') 'feature.json'
...
function Get-FeaturePathsEnv {
    ...
    if ($env:PDCA_FEATURE_DIRECTORY) {
        $featureDir = $env:PDCA_FEATURE_DIRECTORY
        ...
        try {
            ...
            [Console]::Error.WriteLine("ERROR: Failed to parse .pdca/feature.json: $_")
            exit 1
        }
...
function Resolve-Template {
    ...
    $base = Join-Path $RepoRoot '.pdca/templates'
    ...
    $presetsDir = Join-Path $RepoRoot '.pdca/presets'
    ...
    $extDir = Join-Path $RepoRoot '.pdca/extensions'
    if (Test-Path $extDir) {
