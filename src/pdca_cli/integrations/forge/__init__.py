...
- Uses a hyphenated frontmatter `name` value (e.g., `pdca-foo-bar`) for shell compatibility, especially with ZSH
"""
...
def format_forge_command_name(cmd_name: str) -> str:
    ...
    Examples:
        ...
        'pdca-plan'
        >>> format_forge_command_name("pdca.plan")
        'pdca-plan'
        >>> format_forge_command_name("pdca-plan")
        'pdca-plan'
        >>> format_forge_command_name("pdca.my-extension.example")
        'pdca-my-extension-example'
        >>> format_forge_command_name("pdca-my-extension-example")
        'pdca-my-extension-example'
        >>> format_forge_command_name("pdca.jira.sync-status")
        'pdca-jira-sync-status'
    
    Args:
        cmd_name: Command name in dot notation (pdca.foo.bar), 
                  hyphenated format (pdca-foo-bar), or plain name (foo)
    
    Returns:
        Hyphenated command name with 'pdca-' prefix
    """
    # Already in hyphenated format - return as-is (idempotent)
    if cmd_name.startswith("pdca-"):
        return cmd_name
    
    # Strip 'pdca.' prefix if present
    short_name = cmd_name
