...
def format_cline_command_name(cmd_name: str) -> str:
    ...
    Examples:
        ...
        'pdca-plan'
        >>> format_cline_command_name("pdca.plan")
        'pdca-plan'
        >>> format_cline_command_name("pdca.git.commit")
        'pdca-git-commit'
    ...
    if not cmd_name.startswith("pdca-"):
        cmd_name = f"pdca-{cmd_name}"
...
class ClineIntegration(MarkdownIntegration):
    ...
    def command_filename(self, template_name: str) -> str:
        """Cline uses hyphenated filenames (e.g. pdca-git-commit.md)."""
    ...
    def _rewrite_handoff_references(content: str) -> str:
        ...
        return re.sub(
            r"(?m)^(\s*agent:\s*)(pdca\.[A-Za-z0-9-_]+(?:\.[A-Za-z0-9-_]+)*)",
