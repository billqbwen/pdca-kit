"""pdca integration install / uninstall command handlers."""
...
from ._helpers import (
    ...
    _refresh_init_options_pdca_version,
...
def integration_install(
    ...
    if key in installed_keys:
        ...
        if default_key == key:
            ...
            console.print(
                ...
                f"[cyan]pdca integration use {key}[/cyan]."
            )
        console.print(
            f"To refresh its managed files or options, run "
            f"[cyan]pdca integration upgrade {key}[/cyan]."
        )
    ...
    if installed_keys and not force:
        ...
        if unsafe_keys or not getattr(integration, "multi_install_safe", False):
            ...
            console.print(
                ...
                f"[cyan]pdca integration switch {key}[/cyan]."
            )
            console.print(
    ...
    manifest = IntegrationManifest(
        integration.key, project_root, version=_get_pdca_version()
    )
    ...
    try:
        ...
        if new_default == integration.key:
            ...
            _refresh_init_options_pdca_version(project_root)
...
def integration_uninstall(
    ...
    try:
        ...
        console.print(
            ...
            f"[cyan]pdca integration uninstall {key}[/cyan] to clear stale metadata, "
            f"then run [cyan]pdca integration install {key}[/cyan] to regenerate."
        )
