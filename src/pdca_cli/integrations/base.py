...
- ``SkillsIntegration`` — concrete base for integrations that install
  commands as agent skills (``pdca-<name>/SKILL.md`` layout).
"""
...
_HOOK_COMMAND_NOTE = (
    ...
    "For example, `pdca.git.commit` → `/pdca-git-commit`.\n"
)
...
class IntegrationBase(ABC):
    ...
    * ``context_file``     — path (relative to project root) of the agent
    ...
    """Separator used in slash-command invocations (``"."`` → ``/pdca.plan``)."""
    ...
    CONTEXT_MARKER_START = "<!-- PDCA START -->"
    CONTEXT_MARKER_END = "<!-- PDCA END -->"
    ...
    def _resolve_executable(self) -> str:
        ...
        Checks ``PDCA_INTEGRATION_<KEY>_EXECUTABLE`` first, allowing
        ...
        ``PDCA_INTEGRATION_KIRO_CLI_EXECUTABLE``.
        ...
        """
        env_name = (
            f"PDCA_INTEGRATION_{self.key.upper().replace('-', '_')}_EXECUTABLE"
        )
        override = os.environ.get(env_name, "").strip()
        return override if override else self.key

    def _apply_extra_args_env_var(self, args: list[str]) -> None:
        """Append `PDCA_INTEGRATION_<KEY>_EXTRA_ARGS` env-var value to *args*.
        ...
        e.g. `PDCA_INTEGRATION_CLAUDE_EXTRA_ARGS="--dangerously-skip-permissions"`.
        The `INTEGRATION` segment scopes the variable to this subsystem
        so it does not collide with other PDCA Kit env-var namespaces.
        Hyphens in the integration key are replaced with underscores
        and the key is uppercased
        (e.g. `kiro-cli` → `PDCA_INTEGRATION_KIRO_CLI_EXTRA_ARGS`).
        ...
        """
        env_name = (
            f"PDCA_INTEGRATION_{self.key.upper().replace('-', '_')}_EXTRA_ARGS"
        )
    ...
    def dispatch_command(
        ...
        *,
        ...
        """Dispatch a PDCA Kit command through this integration's CLI.
    ...
    def command_filename(self, template_name: str) -> str:
        ...
        *template_name* is the stem of the source file (e.g. ``"plan"``).
        Default: ``pdca.{template_name}.md``.  Subclasses override
        to change the extension or naming convention.
        """
        return f"pdca.{template_name}.md"
    ...
    def install_scripts(
        ...
        ``.pdca/integrations/<key>/scripts/`` in the project.  Shell
        ...
        *manifest*.
        ...
        created: list[Path] = []
        scripts_dest = project_root / ".pdca" / "integrations" / self.key / "scripts"
    ...
    def _agent_context_extension_enabled(project_root: Path) -> bool:
        ...
        """
        registry_path = (
            project_root / ".pdca" / "extensions" / ".registry"
        )
    ...
    def upsert_context_section(
        ...
        start/end markers (default ``<!-- PDCA START -->`` /
        ``<!-- PDCA END -->``) is replaced, or appended when no markers
    ...
    def remove_context_section(self, project_root: Path) -> bool:
        ...
        # For .mdc files, treat PDCA-generated frontmatter-only content as empty
        if ctx_path.suffix == ".mdc":
            import re
    ...
    def resolve_command_refs(content: str, separator: str = ".") -> str:
        """Replace ``__PDCA_COMMAND_<NAME>__`` placeholders with invocations.

        Each placeholder encodes a command name in upper-case with
        underscores (e.g. ``__PDCA_COMMAND_PLAN__``,
        ``__PDCA_COMMAND_GIT_COMMIT__``).  The replacement uses
        *separator* to join the segments:

        * ``separator="."`` → ``/pdca.plan``, ``/pdca.git.commit``
        * ``separator="-"`` → ``/pdca-plan``, ``/pdca-git-commit``
        """
        return re.sub(
            r"__PDCA_COMMAND_([A-Z][A-Z0-9_]*)__",
            lambda m: "/pdca" + separator + m.group(1).lower().replace("_", separator),
            content,
        )
    ...
    def process_template(
        ...
        7. Rewrite paths: ``scripts/`` → ``.pdca/scripts/`` etc.
        8. Replace ``__PDCA_COMMAND_<NAME>__`` with invocation strings
        """
...
class TomlIntegration(IntegrationBase):
    ...
    def command_filename(self, template_name: str) -> str:
        ...
        return f"pdca.{template_name}.toml"
...
class YamlIntegration(IntegrationBase):
    ...
    def command_filename(self, template_name: str) -> str:
        ...
        return f"pdca.{template_name}.yaml"
    ...
    def _human_title(identifier: str) -> str:
        ...
        Strips a leading ``pdca.`` prefix and replaces ``.``, ``-``,
        and ``_`` with spaces before title-casing.
        """
        text = identifier
        if text.startswith("pdca."):
            text = text[len("pdca.") :]
...
class SkillsIntegration(IntegrationBase):
    ...
    Skills use the ``pdca-<name>/SKILL.md`` directory layout following
    ...
    ``pdca-<name>/SKILL.md`` file with skills-oriented frontmatter.
    """
    ...
    def build_command_invocation(self, command_name: str, args: str = "") -> str:
        """Skills use ``/pdca-<stem>`` (hyphenated directory name)."""
        stem = command_name
        if stem.startswith("pdca."):
            stem = stem[len("pdca."):]

        invocation = "/pdca-" + stem.replace(".", "-")
        if args:
            invocation = f"{invocation} {args}"
        return invocation
    ...
    def setup(
        ...
        **opts: Any,
        ...
        Creates ``pdca-<name>/SKILL.md`` for each shared command
        ...
        for src_file in templates:
            ...
            skill_name = f"pdca-{command_name.replace('.', '-')}"
            ...
            description = frontmatter.get("description", "")
            if not description:
                description = f"PDCA Kit: {command_name} workflow"
            ...
            skill_content = (
                ...
                f"compatibility: {_quote('Requires pdca-kit project structure with .pdca/ directory')}\n"
                f"metadata:\n"
                f"  author: {_quote('github-pdca-kit')}\n"
