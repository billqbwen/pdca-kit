...
commands as ``pdca-<name>/SKILL.md`` directories under ``.github/skills/``
...
def _allow_all() -> bool:
    ...
    Checks ``PDCA_COPILOT_ALLOW_ALL_TOOLS`` first (new canonical name).
    Falls back to the deprecated ``PDCA_ALLOW_ALL_TOOLS`` if set,
    emitting a deprecation warning.  Default when neither is set: enabled.
    """
    new_var = os.environ.get("PDCA_COPILOT_ALLOW_ALL_TOOLS")
    if new_var is not None:
        return new_var != "0"

    old_var = os.environ.get("PDCA_ALLOW_ALL_TOOLS")
    if old_var is not None:
        warnings.warn(
            "PDCA_ALLOW_ALL_TOOLS is deprecated; "
            "use PDCA_COPILOT_ALLOW_ALL_TOOLS instead.",
            UserWarning,
            stacklevel=2,
        )
        return old_var != "0"
...
class CopilotIntegration(IntegrationBase):
    ...
    are scaffolded as ``pdca-<name>/SKILL.md`` under ``.github/skills/``
    ...
    def options(cls) -> list[IntegrationOption]:
        return [
            IntegrationOption(
                ...
                is_flag=True,
                default=False,
                help="Scaffold commands as agent skills (pdca-<name>/SKILL.md) instead of .agent.md files",
            ),
        ]
    ...
    def _resolve_executable(self) -> str:
        ...
        Checks ``PDCA_INTEGRATION_COPILOT_EXECUTABLE`` first.  Falls
        ...
        """
        env_name = "PDCA_INTEGRATION_COPILOT_EXECUTABLE"
    ...
    def build_exec_args(
        ...
        *,
        ...
        # Controlled by PDCA_COPILOT_ALLOW_ALL_TOOLS env var
        # (default: enabled).  The deprecated PDCA_ALLOW_ALL_TOOLS
    ...
    def build_command_invocation(self, command_name: str, args: str = "") -> str:
        ...
        Skills mode: ``/pdca-<stem>`` slash-command dispatch.
        """
        if self._skills_mode:
            stem = command_name
    ...
    def dispatch_command(
        ...
        *,
        ...
        """Dispatch via ``--agent pdca.<stem>`` instead of slash-commands.
        ...
        if not skills_mode and project_root:
            ...
            if skills_dir.is_dir():
                skills_mode = any(
                    ...
                    for d in skills_dir.glob("pdca-*")
                )

        if skills_mode:
            prompt = "/pdca-" + stem.replace(".", "-")
            if args:
                prompt = f"{prompt} {args}"
        else:
            agent_name = f"pdca.{stem}"
            prompt = args or ""

        cli_args = [self._resolve_executable(), "-p", prompt]
        # Honour PDCA_INTEGRATION_COPILOT_EXTRA_ARGS for real workflow
    ...
    def command_filename(self, template_name: str) -> str:
        ...
        return f"pdca.{template_name}.agent.md"
    ...
    def setup(
        ...
        **opts: Any,
        ...
        scaffolding (``pdca-<name>/SKILL.md`` under ``.github/skills/``).
