...
def _get_pdca_version() -> str:
    """Return the current PDCA Kit version.

    Resolved lazily through ``_commands.get_pdca_version`` so that tests
    that monkeypatch ``pdca_cli.integrations._commands.get_pdca_version``
    ...
    return _commands.get_pdca_version()
...
def _read_integration_json(project_root: Path) -> dict[str, Any]:
    """Load ``.pdca/integration.json``. Returns normalized state when present.
    ...
    if error.kind == "decode":
        ...
        console.print("Please upgrade PDCA Kit before modifying integrations.")
    raise typer.Exit(1)
...
def _write_integration_json(
    ...
    """Write ``.pdca/integration.json`` with legacy-compatible state."""
    _write_integration_json_file(
        project_root,
        version=_get_pdca_version(),
...
def _refresh_init_options_pdca_version(project_root: Path) -> None:
    """Refresh only the PDCA Kit version recorded in init-options.json."""
    from .. import load_init_options, save_init_options
    opts = load_init_options(project_root)
    if not isinstance(opts, dict) or not opts:
        return
    opts["pdca_version"] = _get_pdca_version()
...
def _remove_integration_json(project_root: Path) -> None:
    """Remove ``.pdca/integration.json`` if it exists."""
    path = project_root / INTEGRATION_JSON
    if path.exists():
        path.unlink()
...
def _resolve_script_type(project_root: Path, script_type: str | None) -> str:
    ...
    if isinstance(saved, str) and saved.strip():
        return _normalize_script_type(saved, ".pdca/init-options.json")
...
def _update_init_options_for_integration(
    ...
    extension config (``.pdca/extensions/agent-context/agent-context-config.yml``),
