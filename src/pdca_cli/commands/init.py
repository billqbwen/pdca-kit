...
def ensure_constitution_from_template(
    ...
    memory_constitution = project_path / ".pdca" / "memory" / "constitution.md"
    template_constitution = project_path / ".pdca" / "templates" / "constitution-template.md"
...
def register(app: typer.Typer) -> None:
    ...
    def init(
        ...
        3. Install bundled PDCA Kit templates, scripts, workflow, and shared
        ...
        with Live(tracker.render(), console=console, refresh_per_second=8, transient=True) as live:
            ...
            try:
                ...
                manifest = IntegrationManifest(
                    resolved_integration.key, project_path, version=get_pdca_version()
                )
                ...
                try:
                    bundled_wf = _locate_bundled_workflow("pdca")
                    if bundled_wf:
                        from ..workflows.catalog import WorkflowRegistry
                        from ..workflows.engine import WorkflowDefinition
                        wf_registry = WorkflowRegistry(project_path)
                        if wf_registry.is_installed("pdca"):
                            tracker.complete("workflow", "already installed")
                        else:
                            import shutil as _shutil
                            dest_wf = project_path / ".pdca" / "workflows" / "pdca"
                            ...
                            wf_registry.add("pdca", {
                                "name": definition.name,
                                "version": definition.version,
                                "description": definition.description,
                                "source": "bundled",
                            })
                            tracker.complete("workflow", "pdca installed")
                    else:
                ...
                if preset:
                    try:
                        ...
                        pdca_ver = get_pdca_version()

                        local_path = Path(preset).resolve()
                        if local_path.is_dir() and (local_path / "preset.yml").exists():
                            preset_manager.install_from_directory(local_path, pdca_ver)
                        else:
                            bundled_path = _locate_bundled_preset(preset)
                            if bundled_path:
                                preset_manager.install_from_directory(bundled_path, pdca_ver)
                            else:
                                ...
                                if not pack_info:
                                    ...
                                    console.print(
                                        f"[yellow]Warning:[/yellow] Preset '{preset}' is bundled with pdca-kit "
                                    ...
                                    try:
                                        ...
                                        preset_manager.install_from_zip(zip_path, pdca_ver)
        ...
        if codex_skill_mode and not ai_skills:
            steps_lines.append(f"{step_num}. Start Codex in this project directory; pdca-kit skills were installed to [cyan].agents/skills[/cyan]")
            step_num += 1
        if claude_skill_mode and not ai_skills:
            steps_lines.append(f"{step_num}. Start Claude in this project directory; pdca-kit skills were installed to [cyan].claude/skills[/cyan]")
            step_num += 1
        if cursor_agent_skill_mode and not ai_skills:
            steps_lines.append(f"{step_num}. Start Cursor Agent in this project directory; pdca-kit skills were installed to [cyan].cursor/skills[/cyan]")
            step_num += 1
        if devin_skill_mode:
            steps_lines.append(f"{step_num}. Start Devin in this project directory; pdca-kit skills were installed to [cyan].devin/skills[/cyan]")
            step_num += 1
