...
class RunState:
    ...
    def load(cls, run_id: str, project_root: Path) -> RunState:
        ...
        runs_dir = project_root / ".pdca" / "workflows" / "runs" / run_id
...
class WorkflowEngine:
    ...
    def execute(
        ...
        run_id:
            Optional run ID (uses PDCA_WORKFLOW_RUN_ID when set, otherwise auto-generated).
        ...
        if effective_run_id is None:
            env_run_id = os.environ.get("PDCA_WORKFLOW_RUN_ID", "").strip()
            if env_run_id:
    ...
    def resume(self, run_id: str) -> RunState:
        ...
        run_dir = self.project_root / ".pdca" / "workflows" / "runs" / run_id
    ...
    def _resolve_default(self, name: str, default: Any) -> Any:
        ...
        recorded in ``.pdca/integration.json`` so workflows dispatch to the
    ...
    def _load_project_integration(self) -> str | None:
        """Read the default integration key from ``.pdca/integration.json``.
    ...
    def list_runs(self) -> list[dict[str, Any]]:
        ...
        runs_dir = self.project_root / ".pdca" / "workflows" / "runs"
