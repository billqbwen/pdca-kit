"""Command step — dispatches a PDCA Kit command to an integration CLI."""
...
class CommandStep(StepBase):
    """Default step type — invokes a PDCA Kit command via the integration CLI.
    ...
    execute the command by name (e.g. ``/pdca.define`` or
    ``/pdca-specify``) rather than reading the file contents.

    .. note::

        CLI output is streamed to the terminal for live progress.
        ``output.exit_code`` is always captured and can be referenced
        by later steps (e.g. ``{{ steps.define.output.exit_code }}``).
    ...
    def _try_dispatch(
        ...
        slash-command invocation (e.g. ``/pdca.define`` for
        markdown agents, ``/pdca-specify`` for skills agents),
