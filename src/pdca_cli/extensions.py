"""
Extension Manager for PDCA Kit

Handles installation, removal, and management of PDCA Kit extensions.
Extensions are modular packages that add commands and functionality to pdca-kit
...
class ExtensionManifest:
    ...
    def _validate(self):
        ...
        if "pdca_version" not in requires:
            raise ValidationError("Missing requires.pdca_version")
        ...
        for cmd in commands:
            ...
            if not EXTENSION_COMMAND_NAME_PATTERN.match(cmd["name"]):
                ...
                if corrected:
                    self.warnings.append(
                        ...
                        f"'pdca.{{extension}}.{{command}}'. Registering as '{corrected}'. "
                    ...
                    raise ValidationError(
                        ...
                        "must follow pattern 'pdca.{extension}.{command}'"
                    )
            ...
            # (e.g. 'pdca.verify' short aliases used by existing extensions).
        ...
        # an alias-form ref (ext.cmd → pdca.ext.cmd).  Always emit a warning when
        ...
        for hook_name, hook_data in self.data.get("hooks", {}).items():
            ...
            # Step 2: lift alias-form '{ext_id}.cmd' to canonical 'pdca.{ext_id}.cmd'.
            parts = after_rename.split(".")
            if len(parts) == 2 and parts[0] == ext["id"]:
                final_ref = f"pdca.{ext['id']}.{parts[1]}"
            else:
    ...
    def _try_correct_command_name(name: str, ext_id: str) -> Optional[str]:
        ...
        Handles the two legacy formats used by community extensions:
          - 'pdca.command'  → 'pdca.{ext_id}.command'
          - '{ext_id}.command' → 'pdca.{ext_id}.command'
        ...
        parts = name.split('.')
        if len(parts) == 2:
            if parts[0] == 'pdca' or parts[0] == ext_id:
                candidate = f"pdca.{ext_id}.{parts[1]}"
...
class ExtensionRegistry:
    ...
    def __init__(self, extensions_dir: Path):
        ...
        Args:
            extensions_dir: Path to .pdca/extensions/ directory
        """
...
class ExtensionManager:
    ...
    def _collect_manifest_command_names(manifest: ExtensionManifest) -> Dict[str, str]:
        ...
        - primary commands must use the canonical `pdca.{extension}.{command}` shape
    ...
    def _register_extension_skills(
        ...
        for cmd_info in manifest.commands:
            ...
            if short_name_raw.startswith("pdca."):
                short_name_raw = short_name_raw[len("pdca."):]
            skill_name = f"pdca-{short_name_raw.replace('.', '-')}"

            # Check if skill already exists before creating the directory
            skill_subdir = skills_dir / skill_name
            skill_file = skill_subdir / "SKILL.md"
            cache_root = extension_dir / ".pdca-dev" / "extension-skills"
    ...
    def check_compatibility(
        ...
        """Check if extension is compatible with current pdca-kit version.
        ...
        try:
            ...
            if current not in specifier:
                raise CompatibilityError(
                    f"Extension requires pdca-kit {required}, "
    ...
    def install_from_zip(
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            return self.install_from_directory(extension_dir, pdca_version, priority=priority)
...
class CommandRegistrar:
    ...
    def register_commands_for_agent(
        ...
        context_note = f"\n<!-- Extension: {manifest.id} -->\n<!-- Config: .pdca/extensions/{manifest.id}/ -->\n"
...
class ExtensionCatalog(CatalogStackBase):
    ...
    DEFAULT_CATALOG_URL = "https://raw.githubusercontent.com/github/pdca-kit/main/extensions/catalog.json"
    COMMUNITY_CATALOG_URL = "https://raw.githubusercontent.com/github/pdca-kit/main/extensions/catalog.community.json"
    ...
    def get_active_catalogs(self) -> List[CatalogEntry]:
        ...
        Resolution order:
        1. PDCA_CATALOG_URL env var — single catalog replacing all defaults
        2. Project-level .pdca/extension-catalogs.yml
        3. User-level ~/.pdca/extension-catalogs.yml
        ...
        # 1. PDCA_CATALOG_URL env var replaces all defaults for backward compat
        if env_value := os.environ.get("PDCA_CATALOG_URL"):
            ...
            return [
                self._entry(
                    ...
                    description="Custom catalog via PDCA_CATALOG_URL",
                )
            ]

        # 2. Project-level config overrides all defaults
        project_config_path = self.project_root / ".pdca" / self.CONFIG_FILENAME
        catalogs = self._load_catalog_config(project_config_path)
        if catalogs is not None:
            return catalogs

        # 3. User-level config
        user_config_path = Path.home() / ".pdca" / self.CONFIG_FILENAME
    ...
    def download_extension(self, extension_id: str, target_dir: Optional[Path] = None) -> Path:
        ...
        if ext_info.get("bundled") and not ext_info.get("download_url"):
            raise ExtensionError(
                f"Extension '{extension_id}' is bundled with pdca-kit and has no download URL. "
...
class ConfigManager:
    ...
    2. Project config (.pdca/extensions/{ext-id}/{ext-id}-config.yml)
    3. Local config (.pdca/extensions/{ext-id}/local-config.yml) - gitignored
    4. Environment variables (PDCA_{EXT_ID}_{KEY})
    """
    ...
    def __init__(self, project_root: Path, extension_id: str):
        ...
        self.extension_dir = project_root / ".pdca" / "extensions" / extension_id
    ...
    def _get_env_config(self) -> Dict[str, Any]:
        ...
        PDCA_{EXT_ID}_{SECTION}_{KEY}

        For example:
        - PDCA_JIRA_CONNECTION_URL
        - PDCA_JIRA_PROJECT_KEY
...
class HookExecutor:
    ...
    def __init__(self, project_root: Path):
        ...
        self.project_root = project_root
        self.extensions_dir = project_root / ".pdca" / "extensions"
        self.config_file = project_root / ".pdca" / "extensions.yml"
    ...
    def _load_init_options(self) -> Dict[str, Any]:
        ...
        Uses the shared helper from pdca_cli and caches values per executor
    ...
    def _skill_name_from_command(command: Any) -> str:
        """Map a command id like pdca.plan to pdca-plan skill name."""
        if not isinstance(command, str):
            return ""
        command_id = command.strip()
        if not command_id.startswith("pdca."):
            return ""
        return f"pdca-{command_id[len('pdca.'):].replace('.', '-')}"
