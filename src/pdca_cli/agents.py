"""
Agent Command Registrar for PDCA Kit
...
def _build_agent_configs() -> dict[str, Any]:
    ...
    for key, integration in INTEGRATION_REGISTRY.items():
        ...
        if integration.registrar_config:
            ...
            # when register_commands() resolves __PDCA_COMMAND_*__ tokens.
...
class CommandRegistrar:
    ...
    def _hyphenate_frontmatter_refs(val: Any) -> Any:
        """Recursively find any dotted references starting with pdca. and hyphenate them."""
        if isinstance(val, dict):
            ...
            return re.sub(
                r"\bpdca\.[A-Za-z0-9-_]+(?:\.[A-Za-z0-9-_]+)*\b",
    ...
    def _hyphenate_body_refs(body: str) -> str:
        """Hyphenate dotted pdca references in command body text."""
        return re.sub(
            r"\bpdca\.[A-Za-z0-9-_]+(?:\.[A-Za-z0-9-_]+)*\b",
            lambda m: m.group(0).replace(".", "-"),
            body,
    ...
    def _adjust_script_paths(self, frontmatter: dict) -> dict:
        ...
        ``memory/``) to the ``.pdca/...`` paths used in generated projects.
    ...
    def rewrite_project_relative_paths(text: str) -> str:
        ...
        for old, new in (
            ("../../memory/", ".pdca/memory/"),
            ("../../scripts/", ".pdca/scripts/"),
            ("../../templates/", ".pdca/templates/"),
        ):
            text = text.replace(old, new)

        # Only rewrite top-level style references so extension-local paths like
        # ".pdca/extensions/<ext>/scripts/..." remain intact.
        text = re.sub(r'(^|[\s`"\'(])(?:\.?/)?memory/', r"\1.pdca/memory/", text)
        text = re.sub(r'(^|[\s`"\'(])(?:\.?/)?scripts/', r"\1.pdca/scripts/", text)
        text = re.sub(
            r'(^|[\s`"\'(])(?:\.?/)?templates/', r"\1.pdca/templates/", text
        )

        return text.replace(".pdca/.pdca/", ".pdca/")
    ...
    def render_yaml_command(
        ...
        """
        from pdca_cli.integrations.base import YamlIntegration
    ...
    def build_skill_frontmatter(
        ...
        skill_frontmatter = {
            ...
            "compatibility": "Requires pdca-kit project structure with .pdca/ directory",
            "metadata": {
                "author": "github-pdca-kit",
                "source": source,
            },
        }
    ...
    def register_commands(
        ...
        for cmd_info in commands:
            ...
            # Resolve __PDCA_COMMAND_*__ tokens using the agent's invoke separator.
            ...
            from pdca_cli.integrations.base import IntegrationBase  # noqa: PLC0415
    ...
    def _write_registered_output(
        ...
        cache_root = source_dir / ".pdca-dev" / "agent-commands" / agent_name
    ...
    def write_copilot_prompt(project_root: Path, cmd_name: str) -> None:
        ...
        Args:
            project_root: Path to project root
            cmd_name: Command name (e.g. 'pdca.my-ext.example')
        """
                ...
                for target_dir in dirs_to_clean:
                    for name in names_to_clean:
                        ...
                        if cmd_file.exists() or cmd_file.is_symlink():
                            ...
                            # subdirectory (e.g. .agents/skills/pdca-ext-cmd/
