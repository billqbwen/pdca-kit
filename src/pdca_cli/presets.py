"""
Preset Manager for PDCA Kit

Handles installation, removal, and management of PDCA Kit presets.
...
def _substitute_core_template(
    ...
    Args:
        ...
        cmd_name: Full command name (e.g. "pdca.git.feature" or "pdca.pdca").
    ...
    # Derive the short name (strip "pdca." prefix) used by core command templates.
    short_name = cmd_name
...
class PresetManifest:
    ...
    def _validate(self):
        ...
        if "pdca_version" not in requires:
            raise PresetValidationError("Missing requires.pdca_version")
        ...
        for tmpl in provides["templates"]:
            ...
            if tmpl["type"] == "command":
                # Commands use dot notation (e.g. pdca.define)
    ...
    @property
    def requires_pdca_version(self) -> str:
        """Get required pdca-kit version range."""
        return self.data["requires"]["pdca_version"]
...
class PresetRegistry:
    ...
    def __init__(self, packs_dir: Path):
        ...
        Args:
            packs_dir: Path to .pdca/presets/ directory
        """
...
class PresetManager:
    ...
    def __init__(self, project_root: Path):
        ...
        self.presets_dir = project_root / ".pdca" / "presets"
    ...
    def check_compatibility(
        ...
        """Check if preset is compatible with current pdca-kit version.

        Args:
            manifest: Preset manifest
            pdca_version: Current pdca-kit version
        ...
        try:
            ...
            if current not in specifier:
                raise PresetCompatibilityError(
                    f"Preset requires pdca-kit {required}, "
                    f"but {pdca_version} is installed.\n"
                    f"Upgrade pdca-kit with: {REINSTALL_COMMAND}"
                )
    ...
    def _register_commands(
        ...
        # Command names follow the pattern: pdca.<ext-id>.<cmd-name>
        # Core commands (e.g. pdca.define) have only one dot — always register.
        extensions_dir = self.project_root / ".pdca" / "extensions"
        filtered = []
        for cmd in command_templates:
            parts = cmd["name"].split(".")
            if len(parts) >= 3 and parts[0] == "pdca":
                ext_id = parts[1]
            ...
            if not has_composition:
                ...
                if not registered:
                    ...
                    if source.startswith("extension:"):
                        ...
                        ext_dir = self.project_root / ".pdca" / "extensions" / ext_id
    ...
    def _register_command_from_path(
        ...
        if source_id and not source_id.startswith("preset:"):
            try:
                from .extensions import ExtensionManifest
                for ext_dir in (self.project_root / ".pdca" / "extensions").iterdir():
    ...
    def _skill_names_for_command(cmd_name: str) -> tuple[str, str]:
        ...
        modern_skill_name = f"pdca-{raw_short_name.replace('.', '-')}"
        legacy_skill_name = f"pdca.{raw_short_name}"
    ...
    def _skill_title_from_command(cmd_name: str) -> str:
        ...
        title_name = cmd_name
        if title_name.startswith("pdca."):
            title_name = title_name[len("pdca."):]
    ...
    def _resolve_skill_command_refs(
        ...
        """Render ``__PDCA_COMMAND_*__`` tokens in a skill body as invocations.

        Looks up the agent's invoke separator and rewrites each
        ``__PDCA_COMMAND_<NAME>__`` placeholder into the matching
        slash-command invocation — ``/pdca-<cmd>`` for a ``-`` separator,
        ``/pdca.<cmd>`` for ``.`` — the same rendering the command layer
    ...
    def _unregister_skills(self, skill_names: List[str], preset_dir: Path) -> None:
        ...
        core_templates_dir = self.project_root / ".pdca" / "templates" / "commands"
        init_opts = load_init_options(self.project_root)
        if not isinstance(init_opts, dict):
            init_opts = {}
        selected_ai = init_opts.get("ai")
        ...
        for skill_name in skill_names:
            ...
            if short_name.startswith("pdca-"):
                short_name = short_name[len("pdca-"):]
            elif short_name.startswith("pdca."):
                short_name = short_name[len("pdca."):]
    ...
    def install_from_directory(
        ...
        self.check_compatibility(manifest, pdca_version)
    ...
    def install_from_zip(
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            return self.install_from_directory(pack_dir, pdca_version, priority)
...
class PresetCatalog:
    ...
    DEFAULT_CATALOG_URL = "https://raw.githubusercontent.com/github/pdca-kit/main/presets/catalog.json"
    COMMUNITY_CATALOG_URL = "https://raw.githubusercontent.com/github/pdca-kit/main/presets/catalog.community.json"
    ...
    def __init__(self, project_root: Path):
        ...
        Args:
            project_root: Root directory of the pdca-kit project
        """
        self.project_root = project_root
        self.presets_dir = project_root / ".pdca" / "presets"
    ...
    def _make_request(self, url: str):
        ...
        Delegates to :func:`pdca_cli.authentication.http.build_request`.
        """
        from pdca_cli.authentication.http import build_request
        return build_request(url)

    def _open_url(self, url: str, timeout: int = 10):
        """Open a URL with provider-based auth, trying each configured provider.

        Delegates to :func:`pdca_cli.authentication.http.open_url`.
        """
        from pdca_cli.authentication.http import open_url
    ...
    def get_active_catalogs(self) -> List[PresetCatalogEntry]:
        ...
        Resolution order:
        1. PDCA_PRESET_CATALOG_URL env var — single catalog replacing all defaults
        2. Project-level .pdca/preset-catalogs.yml
        3. User-level ~/.pdca/preset-catalogs.yml
        ...
        # 1. PDCA_PRESET_CATALOG_URL env var replaces all defaults
        if env_value := os.environ.get("PDCA_PRESET_CATALOG_URL"):
            ...
            return [PresetCatalogEntry(url=catalog_url, name="custom", priority=1, install_allowed=True, description="Custom catalog via PDCA_PRESET_CATALOG_URL")]

        # 2. Project-level config overrides all defaults
        project_config_path = self.project_root / ".pdca" / "preset-catalogs.yml"
...
class PresetResolver:
    ...
    Resolution order:
    1. .pdca/templates/overrides/          - Project-local overrides
    2. .pdca/presets/<preset-id>/          - Installed presets
    ...
    def __init__(self, project_root: Path):
        ...
        """
        self.project_root = project_root
        self.templates_dir = project_root / ".pdca" / "templates"
        self.presets_dir = project_root / ".pdca" / "presets"
        self.overrides_dir = self.templates_dir / "overrides"
        self.extensions_dir = project_root / ".pdca" / "extensions"
    ...
    def _core_stem(template_name: str) -> Optional[str]:
        ...
        or ``None`` otherwise.
        """
        if template_name.startswith("pdca."):
            return template_name[len("pdca."):]
        return None
    ...
    def resolve(
        ...
        # pdca's built-in command/template files and must always be checked
        # so that strategy:wrap presets can locate {CORE_TEMPLATE}.
        from pdca_cli import _locate_core_pack  # local import to avoid cycles
    ...
    def resolve_extension_command_via_manifest(self, cmd_name: str) -> Optional[Path]:
        ...
        (e.g. ``pdca.selftest.extension`` → ``commands/selftest.md``).
    ...
    def _find_bundled_core(
        ...
        ``.pdca/templates/`` doesn't contain the core file.
        """
        try:
            from pdca_cli import _locate_core_pack
        except ImportError:
            return None
