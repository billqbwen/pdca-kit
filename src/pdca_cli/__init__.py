...
def catalog_list():
    ...
    config_path = project_root / ".pdca" / "extension-catalogs.yml"
    user_config_path = Path.home() / ".pdca" / "extension-catalogs.yml"
    if os.environ.get("PDCA_CATALOG_URL"):
        console.print("[dim]Catalog configured via PDCA_CATALOG_URL environment variable.[/dim]")
    else:
        try:
            proj_loaded = config_path.exists() and catalog._load_catalog_config(config_path) is not None
        ...
        if proj_loaded:
            ...
            try:
                ...
                user_loaded = False
            if user_loaded:
                console.print("[dim]Config: ~/.pdca/extension-catalogs.yml[/dim]")
            else:
                console.print("[dim]Using built-in default catalog stack.[/dim]")
                console.print(
                    "[dim]Add .pdca/extension-catalogs.yml to customize.[/dim]"
                )
    ...
    try:
        with console.status(f"[cyan]Installing extension: {extension}[/cyan]"):
            if dev:
                ...
                download_dir = project_root / ".pdca" / "extensions" / ".cache" / "downloads"
                ...
                try:
                    ...
                    manifest = manager.install_from_zip(zip_path, pdca_version, priority=priority)
                except urllib.error.URLError as e:
                ...
                # Try bundled extensions first (shipped with pdca-kit)
                bundled_path = _locate_bundled_extension(extension)
                if bundled_path is not None:
                    manifest = manager.install_from_directory(bundled_path, pdca_version, priority=priority)
                else:
                    ...
                    if bundled_path is None:
                        ...
                        if ext_info.get("bundled") and not ext_info.get("download_url"):
                            console.print(
                                f"[red]Error:[/red] Extension '{ext_info['id']}' is bundled with pdca-kit "
                                f"but could not be found in the installed package."
                            )
                            console.print(
                                "\nThis usually means the pdca-kit installation is incomplete or corrupted."
                        ...
                        if not ext_info.get("_install_allowed", True):
                            ...
                            console.print(
                                ...
                                f"(install_allowed: true) in .pdca/extension-catalogs.yml."
                            )
                            raise typer.Exit(1)
        ...
        console.print(f"   Check: .pdca/extensions/{manifest.id}/")
...
def extension_remove(
    ...
    if not force:
        ...
        console.print(f"   • Extension directory: .pdca/extensions/{extension_id}/")
        if not keep_config:
...
def _print_extension_info(ext_info: dict, manager):
    ...
    if ext_info.get('requires'):
        ...
        if reqs.get('pdca_version'):
            console.print(f"  • PDCA Kit: {reqs['pdca_version']}")
        if reqs.get('tools'):
    ...
    if is_installed:
        ...
        console.print(
            ...
            f"but not in your approved catalog. Add it to .pdca/extension-catalogs.yml "
        ...
        for update in updates_available:
            ...
            try:
                ...
                try:
                    ...
                    _ = manager.install_from_zip(zip_path, pdca_version)
...
def workflow_remove(
    ...
    workflow_dir = project_root / ".pdca" / "workflows" / workflow_id
    if workflow_dir.exists():
        import shutil
