...
from pdca_cli.extensions import ExtensionRegistry
...
class TestPresetManifest:
    ...
    def test_valid_manifest(self, pack_dir):
        ...
        assert manifest.requires_pdca_version == ">=0.1.0"
    ...
    def test_missing_pdca_version(self, temp_dir, valid_pack_data):
        """Test missing requires.pdca_version."""
        del valid_pack_data["requires"]["pdca_version"]
        manifest_path = temp_dir / "preset.yml"
        with open(manifest_path, 'w') as f:
            yaml.dump(valid_pack_data, f)
        with pytest.raises(PresetValidationError, match="Missing requires.pdca_version"):
            PresetManifest(manifest_path)
...
class TestPresetManager:
    ...
    def test_install_from_directory(self, project_dir, pack_dir):
        ...
        installed_dir = project_dir / ".pdca" / "presets" / "test-pack"
    ...
    def test_install_incompatible(self, project_dir, temp_dir, valid_pack_data):
        ...
        valid_pack_data["requires"]["pdca_version"] = ">=99.0.0"
    ...
    def test_check_compatibility_invalid(self, pack_dir, temp_dir):
        ...
        manager = PresetManager(temp_dir)
        manifest = PresetManifest(pack_dir / "preset.yml")
        manifest.data["requires"]["pdca_version"] = "not-a-specifier"
...
class TestPresetResolver:
    ...
    def test_resolve_override_takes_priority(self, project_dir):
        ...
        # Create override
        overrides_dir = project_dir / ".pdca" / "templates" / "overrides"
    ...
    def test_resolve_extension_provided_templates(self, project_dir):
        ...
        ext_dir = project_dir / ".pdca" / "extensions" / "my-ext"
    ...
    def test_resolve_disabled_extension_templates_skipped(self, project_dir):
        ...
        # Create extension with templates
        ext_dir = project_dir / ".pdca" / "extensions" / "disabled-ext"
        ext_templates_dir = ext_dir / "templates"
        ext_templates_dir.mkdir(parents=True)
    ...
    def test_resolve_disabled_extension_not_picked_up_as_unregistered(self, project_dir):
        ...
        ext_dir = project_dir / ".pdca" / "extensions" / "test-disabled-ext"
        ext_templates_dir = ext_dir / "templates"
        ext_templates_dir.mkdir(parents=True)
    ...
    def test_resolve_skips_hidden_extension_dirs(self, project_dir):
        ...
        ext_dir = project_dir / ".pdca" / "extensions" / ".backup"
        ext_templates_dir = ext_dir / "templates"
        ext_templates_dir.mkdir(parents=True)
...
class TestResolveCore:
    ...
    def test_resolve_core_returns_extension_template(self, project_dir):
        ...
        ext_cmd_dir = project_dir / ".pdca" / "extensions" / "myext" / "commands"
    ...
    def test_resolve_extension_command_via_manifest_skips_oserror_manifests(self, project_dir):
        ...
        ext_dir = project_dir / ".pdca" / "extensions" / "bad-ext"
        ...
        with mock.patch("builtins.open", side_effect=PermissionError("denied")):
            result = resolver.resolve_extension_command_via_manifest("pdca.bad-ext.mycmd")
...
class TestPresetCatalog:
    ...
    def test_search_with_cached_data(self, project_dir, monkeypatch):
        ...
        monkeypatch.delenv("PDCA_PRESET_CATALOG_URL", raising=False)
        catalog = PresetCatalog(project_dir)
        catalog.cache_dir.mkdir(parents=True, exist_ok=True)
    ...
    def test_env_var_catalog_url(self, project_dir, monkeypatch):
        ...
        monkeypatch.setenv("PDCA_PRESET_CATALOG_URL", "https://custom.example.com/catalog.json")
        catalog = PresetCatalog(project_dir)
...
class TestPresetCatalogMultiCatalog:
    ...
    def test_env_var_overrides_catalogs(self, project_dir, monkeypatch):
        """Test that PDCA_PRESET_CATALOG_URL env var overrides defaults."""
        monkeypatch.setenv(
            "PDCA_PRESET_CATALOG_URL",
    ...
    def test_project_config_overrides_defaults(self, project_dir):
        ...
        config_path = project_dir / ".pdca" / "preset-catalogs.yml"
    ...
    def test_load_catalog_config_nonexistent(self, project_dir):
        ...
        result = catalog._load_catalog_config(
            project_dir / ".pdca" / "nonexistent.yml"
        )
        assert result is None
...
SELF_TEST_WRAP_WARNING = (
    r"Cannot compose command 'pdca\.wrap-test': no base layer\. "
...
def install_self_test_preset(manager: PresetManager, pdca_version: str = "0.1.5") -> PresetManifest:
    ...
    with warnings.catch_warnings():
        warnings.filterwarnings(
            ...
            module=r"pdca_cli\.presets",
        )
        return manager.install_from_directory(SELF_TEST_PRESET_DIR, pdca_version)


class TestSelfTestPreset:
    """Tests using the self-test preset that ships with the repo.

    The self-test preset ships a wrap-strategy command (``pdca.wrap-test``)
    ...
    def test_self_test_overrides_all_core_templates(self, project_dir):
        ...
        templates_dir = project_dir / ".pdca" / "templates"
        for name in CORE_TEMPLATE_NAMES:
    ...
    def test_self_test_command_file_exists(self):
        ...
        cmd_path = SELF_TEST_PRESET_DIR / "commands" / "pdca.define.md"
    ...
    def test_self_test_registers_commands_for_gemini(self, project_dir):
        ...
        cmd_file = gemini_dir / "pdca.define.toml"
    ...
    def test_extension_command_skipped_when_extension_missing(self, project_dir, temp_dir):
        ...
        (preset_dir / "commands" / "pdca.fakeext.cmd.md").write_text(
        ...
        manifest_data = {
            ...
            "provides": {
                "templates": [
                    {
                        "type": "command",
                        "name": "pdca.fakeext.cmd",
                        "file": "commands/pdca.fakeext.cmd.md",
                        "description": "Override fakeext cmd",
                    }
                ]
            },
        ...
        cmd_file = claude_dir / "pdca.fakeext.cmd.md"
    ...
    def test_extension_command_registered_when_extension_present(self, project_dir, temp_dir):
        ...
        claude_dir = project_dir / ".claude" / "skills"
        claude_dir.mkdir(parents=True)
        (project_dir / ".pdca" / "extensions" / "fakeext").mkdir(parents=True)
        ...
        cmd_file = claude_dir / "pdca-fakeext-cmd" / "SKILL.md"
...
class TestInitOptions:
    ...
    def test_save_and_load_round_trip(self, project_dir):
        from pdca_cli import save_init_options, load_init_options
    ...
    def test_load_returns_empty_when_missing(self, project_dir):
        from pdca_cli import load_init_options
    ...
    def test_load_returns_empty_on_invalid_json(self, project_dir):
        ...
        opts_file = project_dir / ".pdca" / "init-options.json"
    ...
    def test_save_writes_real_utf8_bytes(self, project_dir):
        ...
        """
        from pdca_cli import save_init_options
...
class TestPresetSkills:
    ...
    def test_skill_overridden_on_preset_install(self, project_dir, temp_dir):
        ...
        # Install self-test preset (has a command override for pdca.define)
        manager = PresetManager(project_dir)
        install_self_test_preset(manager)
    ...
    def test_register_skills_resolves_command_refs(self, project_dir, temp_dir):
        ...
        instead of rendering as ``/pdca-<cmd>`` like the command layer.
        """
        self._write_init_options(project_dir, ai="claude")
        skills_dir = project_dir / ".claude" / "skills"
        self._create_skill(skills_dir, "pdca-specify")
    ...
    def test_reconcile_override_skill_resolves_command_refs(self, project_dir, temp_dir):
        ...
        the override body — which must also render ``__PDCA_COMMAND_*__`` tokens.
        """
        self._write_init_options(project_dir, ai="claude")
        skills_dir = project_dir / ".claude" / "skills"
        self._create_skill(skills_dir, "pdca-specify")
        ...
        (overrides_dir / "pdca.define.md").write_text(
    ...
    def test_extension_restore_resolves_command_refs(self, project_dir, temp_dir):
        ...
        extension command body, ``__PDCA_COMMAND_*__`` tokens in that body
        must render as slash-command invocations like the core-template path.
        """
        self._write_init_options(project_dir, ai="claude")
        skills_dir = project_dir / ".claude" / "skills"
        self._create_skill(skills_dir, "pdca-fakeext-cmd", body="original extension skill")

        extension_dir = project_dir / ".pdca" / "extensions" / "fakeext"
        ...
        content = (skills_dir / "pdca-fakeext-cmd" / "SKILL.md").read_text()
        assert "source: extension:fakeext" in content, "skill should be restored from the extension"
        assert "__PDCA_COMMAND_" not in content, "raw command token leaked on extension restore"
        assert "/pdca-plan" in content
    ...
    def test_core_command_override_skill_uses_preset_command_description(self, project_dir, temp_dir):
        ...
        self._write_init_options(project_dir, ai="claude")
        skills_dir = project_dir / ".claude" / "skills"
        self._create_skill(skills_dir, "pdca-taskstoissues")

        preset_dir = temp_dir / "taskstoissues-description"
        preset_dir.mkdir()
        (preset_dir / "commands").mkdir()
        (preset_dir / "commands" / "pdca.repro.taskstoissues.md").write_text(
            "---\n"
        ...
        manifest_data = {
            ...
            "provides": {
                "templates": [
                    {
                        "type": "command",
                        "name": "pdca.taskstoissues",
                        "file": "commands/pdca.repro.taskstoissues.md",
                        "description": "MANIFEST-DESCRIPTION",
                        "replaces": "pdca.taskstoissues",
                        "strategy": "replace",
                    }
                ]
            },
        ...
        skill_file = skills_dir / "pdca-taskstoissues" / "SKILL.md"
        content = skill_file.read_text()
    ...
    def test_override_skill_reconcile_uses_override_command_description(self, project_dir, temp_dir):
        ...
        overrides_dir = project_dir / ".pdca" / "templates" / "overrides"
        overrides_dir.mkdir(parents=True)
        (overrides_dir / "pdca.taskstoissues.md").write_text(
            "---\n"
        ...
        assert "Convert tasks from tasks.md into GitHub issues." not in content
        assert "source: override:pdca.taskstoissues" in content
    ...
    def test_skill_restored_on_remove_resolves_script_placeholders(self, project_dir):
        ...
        (core_cmds / "specify.md").write_text(
            ...
            "description: Core specify command\n"
            "scripts:\n"
            "  sh: .pdca/scripts/bash/create-new-feature.sh --json \"{ARGS}\"\n"
            "---\n\n"
            "Run:\n"
            "{SCRIPT}\n"
        )
    ...
    def test_extension_skill_override_matches_hyphenated_multisegment_name(self, project_dir, temp_dir):
        """Preset overrides for pdca.<ext>.<cmd> should target pdca-<ext>-<cmd> skills."""
        self._write_init_options(project_dir, ai="codex")
        skills_dir = project_dir / ".agents" / "skills"
        self._create_skill(skills_dir, "pdca-fakeext-cmd", body="untouched")
        (project_dir / ".pdca" / "extensions" / "fakeext").mkdir(parents=True, exist_ok=True)
        ...
        skill_file = skills_dir / "pdca-fakeext-cmd" / "SKILL.md"
        assert skill_file.exists()
        content = skill_file.read_text()
        assert "preset:ext-skill-override" in content
        assert "name: pdca-fakeext-cmd" in content
        assert "# PDCA Fakeext Cmd Skill" in content

        metadata = manager.registry.get("ext-skill-override")
        assert "pdca-fakeext-cmd" in metadata.get("registered_skills", [])
    ...
    def test_preset_remove_skips_skill_dir_without_skill_file(self, project_dir, temp_dir):
        ...
        self._write_init_options(project_dir, ai="codex")
        skills_dir = project_dir / ".agents" / "skills"
        stray_skill_dir = skills_dir / "pdca-fakeext-cmd"
        ...
        manager.registry.add(
            "ext-skill-missing-file",
            {
                ...
                "provides_templates": ["pdca.fakeext.cmd"],
                "registered_skills": ["pdca-fakeext-cmd"],
                "priority": 10,
            },
        )
    ...
    def test_kimi_new_skill_created_even_when_ai_skills_disabled(self, project_dir, temp_dir):
        ...
        preset_dir.mkdir()
        (preset_dir / "commands").mkdir()
        (preset_dir / "commands" / "pdca.research.md").write_text(
            "---\n"
                ...
                "templates": [
                    {
                        "type": "command",
                        "name": "pdca.research",
                        "file": "commands/pdca.research.md",
                    }
                ]
            },
        }
        ...
        skill_file = skills_dir / "pdca-research" / "SKILL.md"
        assert skill_file.exists()
        content = skill_file.read_text()
        assert "preset:kimi-new-skill" in content
        assert "name: pdca-research" in content

        metadata = manager.registry.get("kimi-new-skill")
        assert "pdca-research" in metadata.get("registered_skills", [])
    ...
    def test_kimi_preset_skill_override_resolves_script_placeholders(self, project_dir, temp_dir):
        ...
        preset_dir.mkdir()
        (preset_dir / "commands").mkdir()
        (preset_dir / "commands" / "pdca.define.md").write_text(
            "---\n"
        ...
        assert ".pdca/scripts/bash/create-new-feature.sh --json \"$ARGUMENTS\"" in content
        assert ".pdca/templates/checklist.md" in content
        assert ".pdca/memory/constitution.md" in content
...
class TestPresetPriorityBackwardsCompatibility:
    ...
    def test_legacy_preset_without_priority_field(self, temp_dir):
        ...
        presets_dir = temp_dir / ".pdca" / "presets"
...
class TestPresetEnableDisable:
    ...
    def test_disabled_preset_excluded_from_resolution(self, project_dir, pack_dir):
        ...
        preset_template = project_dir / ".pdca" / "presets" / "test-pack" / "templates" / "test-template.md"
...
class TestLeanPreset:
    ...
    def test_lean_commands_have_no_scripts(self):
        ...
        for name in LEAN_COMMAND_NAMES:
            cmd_path = LEAN_PRESET_DIR / "commands" / f"pdca.{name.split('.')[-1]}.md"
            content = cmd_path.read_text()
...
class TestBundledPresetLocator:
    ...
    def test_locate_bundled_lean_preset(self):
        ...
        from pdca_cli import _locate_bundled_preset
    ...
    def test_bundled_preset_add_via_cli(self, project_dir):
        ...
        runner = CliRunner()
        with patch.object(Path, "cwd", return_value=project_dir), \
             patch("pdca_cli.get_pdca_version", return_value="0.6.0"):
    ...
    def test_bundled_preset_download_raises_error(self, project_dir):
        ...
        with patch.object(catalog, "_get_merged_packs", return_value=catalog_data):
            with pytest.raises(PresetError, match="bundled with pdca-kit"):
    ...
    def test_bundled_preset_missing_locally_cli_error(self, project_dir):
        ...
        with patch.object(Path, "cwd", return_value=project_dir), \
             patch("pdca_cli._locate_bundled_preset", return_value=None), \
             patch("pdca_cli.presets.PresetCatalog") as MockCatalog:
...
class TestWrapStrategy:
    ...
    def test_substitute_core_template_replaces_placeholder(self, project_dir):
        ...
        from pdca_cli.presets import _substitute_core_template
        from pdca_cli.agents import CommandRegistrar
    ...
    def test_register_commands_substitutes_core_template_for_wrap_strategy(self, project_dir):
        ...
        (cmd_dir / "pdca.define.md").write_text(
        ...
        written = (agent_dir / "pdca.define.md").read_text()
    ...
    def test_end_to_end_wrap_via_self_test_preset(self, project_dir):
        ...
        from pdca_cli.presets import PresetManager
    ...
    def test_register_commands_toml_resolves_inherited_scripts(self, project_dir):
        ...
        written = (toml_dir / "pdca.define.toml").read_text()
        assert "{CORE_TEMPLATE}" not in written
        assert "{SCRIPT}" not in written
        assert "run.sh" in written
    ...
    def test_extension_command_resolves_via_extension_directory(self, project_dir):
        """Extension commands (e.g. pdca.git.feature) resolve from the extension directory.
        ...
        and finds pdca.git.feature.md in the extension commands directory.
        """
        from pdca_cli.presets import _substitute_core_template
        from pdca_cli.agents import CommandRegistrar

        # Place the template where a real extension would install it
        ext_cmd_dir = project_dir / ".pdca" / "extensions" / "git" / "commands"
        ext_cmd_dir.mkdir(parents=True, exist_ok=True)
        (ext_cmd_dir / "pdca.git.feature.md").write_text(
            "---\ndescription: git feature core\n---\n\n# Git Feature Core\n"
        )
        # Ensure a hyphenated or dot-separated fallback does NOT exist
        assert not (project_dir / ".pdca" / "templates" / "commands" / "git.feature.md").exists()
        assert not (project_dir / ".pdca" / "templates" / "commands" / "git-feature.md").exists()
        ...
        result, _ = _substitute_core_template(body, "pdca.git.feature", project_dir, registrar)
    ...
    def test_extension_command_resolves_via_manifest_when_filename_differs(self, project_dir):
        ...
        The selftest extension maps pdca.selftest.extension → commands/selftest.md.
        Name-based lookup would look for commands/pdca.selftest.extension.md and fail;
        manifest-based lookup must find the actual file declared in the manifest.
        """
        from pdca_cli.presets import _substitute_core_template
        from pdca_cli.agents import CommandRegistrar

        ext_dir = project_dir / ".pdca" / "extensions" / "selftest"
        cmd_dir = ext_dir / "commands"
        cmd_dir.mkdir(parents=True, exist_ok=True)

        # File is named selftest.md, NOT pdca.selftest.extension.md
        ...
        registrar = CommandRegistrar()
        body = "## Wrapper\n\n{CORE_TEMPLATE}\n"
        result, _ = _substitute_core_template(body, "pdca.selftest.extension", project_dir, registrar)
...
class TestResolveContent:
    ...
    def test_resolve_content_wrap_strategy_script(self, project_dir, temp_dir, valid_pack_data):
        ...
        # Create core script
        scripts_dir = project_dir / ".pdca" / "templates" / "scripts"
    ...
    def test_resolve_content_command_type(self, project_dir, temp_dir, valid_pack_data):
        ...
        # Create core command using stem naming (matches real layout: plan.md, not pdca.plan.md)
        commands_dir = project_dir / ".pdca" / "templates" / "commands"
        ...
        (pack_dir / "commands" / "pdca.plan.md").write_text("## Additional Instructions\n")

        manager = PresetManager(project_dir)
        manager.install_from_directory(pack_dir, "0.1.5")

        resolver = PresetResolver(project_dir)
        content = resolver.resolve_content("pdca.plan", "command")
        assert content is not None
    ...
    def test_resolve_content_command_frontmatter_stripping(self, project_dir, temp_dir, valid_pack_data):
        ...
        with open(pack_dir / "preset.yml", 'w') as f:
            yaml.dump(pack_data, f)
        (pack_dir / "commands").mkdir()
        (pack_dir / "commands" / "pdca.check.md").write_text(
        ...
        resolver = PresetResolver(project_dir)
        content = resolver.resolve_content("pdca.check", "command")
        assert content is not None
...
class TestRemoveReconciliation:
    ...
    def test_remove_restores_lower_priority_command(
        ...
        (lo_dir / "commands" / "pdca.define.md").write_text(
            "---\ndescription: lo\n---\nLo content\n"
        )
        ...
        (hi_dir / "commands" / "pdca.define.md").write_text(
            "---\ndescription: hi\n---\nHi content\n"
        )
