...
_RATE_LIMITED_REASON = (
    "rate limited (configure ~/.pdca/auth.json with a GitHub token)"
)
...
def _http_error(code: int, message: str = "error") -> urllib.error.HTTPError:
    return urllib.error.HTTPError(
        url="https://api.github.com/repos/github/pdca-kit/releases/latest",
        code=code,
        msg=message,
...
class TestSelfUpgradeStub:
    ...
    def test_stub_makes_no_network_call(self):
        ...
        with (
            patch(
                "pdca_cli.authentication.http.urllib.request.urlopen",
                side_effect=network_error,
            ),
            patch(
                "pdca_cli.authentication.http.urllib.request.build_opener",
                side_effect=network_error,
            ),
        ):
            result = runner.invoke(app, ["self", "upgrade"])
...
class TestUserStory1:
    def test_newer_available_prints_update_and_install_command(self):
        with patch("pdca_cli._version._get_installed_version", return_value="0.7.4"), patch(
            "pdca_cli.authentication.http.urllib.request.urlopen",
        ...
        assert "git+https://github.com/github/pdca-kit.git@v0.9.0" in output

    def test_up_to_date_prints_current_only(self):
        with patch("pdca_cli._version._get_installed_version", return_value="0.9.0"), patch(
            "pdca_cli.authentication.http.urllib.request.urlopen",
            return_value=_mock_urlopen_response({"tag_name": "v0.9.0"}),
        ):
            result = runner.invoke(app, ["self", "check"])
    ...
    def test_dev_build_ahead_of_release_is_up_to_date(self):
        with patch("pdca_cli._version._get_installed_version", return_value="0.7.5.dev0"), patch(
            "pdca_cli.authentication.http.urllib.request.urlopen",
    ...
    def test_unknown_installed_still_prints_latest_and_reinstall(self):
        with patch("pdca_cli._version._get_installed_version", return_value="unknown"), patch(
            "pdca_cli.authentication.http.urllib.request.urlopen",
            return_value=_mock_urlopen_response({"tag_name": "v0.7.4"}),
        ):
            result = runner.invoke(app, ["self", "check"])
        ...
        assert "0.7.4" in output
        assert "git+https://github.com/github/pdca-kit.git@v0.7.4" in output
...
class TestUserStory2:
    ...
    def test_failure_prints_installed_plus_one_line_reason(
        ...
        with patch("pdca_cli._version._get_installed_version", return_value="0.7.4"), patch(
            "pdca_cli.authentication.http.urllib.request.urlopen", side_effect=side_effect
        ):
            result = runner.invoke(app, ["self", "check"])
        output = strip_ansi(result.output)
        ...
        if expected_reason == _RATE_LIMITED_REASON:
            ...
            assert "~/.pdca/auth.json" in output
        else:
