...
def project_dir(temp_dir):
    """Create a mock pdca-kit project directory."""
    proj_dir = temp_dir / "project"
    proj_dir.mkdir()

    # Create .pdca directory
    specify_dir = proj_dir / ".pdca"
    specify_dir.mkdir()
...
class TestExtensionManifest:
    ...
    def test_valid_manifest(self, extension_dir):
        ...
        assert manifest.commands[0]["name"] == "pdca.test-ext.hello"
    ...
    def test_command_name_autocorrect_pdca_prefix(self, temp_dir, valid_manifest_data):
        """Test that 'pdca.command' is auto-corrected to 'pdca.{ext_id}.command'."""
        import yaml

        valid_manifest_data["provides"]["commands"][0]["name"] = "pdca.hello"
        ...
        assert "pdca.hello" in manifest.warnings[0]
        assert "pdca.test-ext.hello" in manifest.warnings[0]

    def test_command_name_autocorrect_matching_ext_id_prefix(self, temp_dir, valid_manifest_data):
        """Test that '{ext_id}.command' is auto-corrected to 'pdca.{ext_id}.command'."""
        import yaml
        ...
        assert manifest.commands[0]["name"] == "pdca.docguard.guard"
        assert len(manifest.warnings) == 1
        assert "docguard.guard" in manifest.warnings[0]
        assert "pdca.docguard.guard" in manifest.warnings[0]
    ...
    def test_alias_free_form_accepted(self, temp_dir, valid_manifest_data):
        """Aliases are free-form — a 'pdca.command' alias must be accepted unchanged."""
        import yaml

        valid_manifest_data["provides"]["commands"][0]["aliases"] = ["pdca.hello"]
        ...
        assert manifest.commands[0]["aliases"] == ["pdca.hello"]
    ...
    def test_non_dict_hook_entry_raises_validation_error(self, temp_dir, valid_manifest_data):
        ...
        valid_manifest_data["hooks"]["after_tasks"] = "pdca.test-ext.hello"
...
class TestExtensionManager:
    ...
    def test_check_compatibility_invalid(self, extension_dir, project_dir):
        ...
        with pytest.raises(CompatibilityError, match="Extension requires pdca-kit"):
    ...
    def test_install_from_directory(self, extension_dir, project_dir):
        ...
        ext_dir = project_dir / ".pdca" / "extensions" / "test-ext"
    ...
    def test_install_rejects_extension_id_in_core_namespace(self, temp_dir, project_dir):
        ...
        manifest_data = {
            ...
            "requires": {"pdca_version": ">=0.1.0"},
            "provides": {
                "commands": [
                    {
                        "name": "pdca.analyze.extra",
    ...
    def test_install_accepts_free_form_alias(self, temp_dir, project_dir):
        """Aliases are free-form — a short 'pdca.shortcut' alias must be preserved unchanged."""
        import yaml
        ...
        manifest_data = {
            ...
            "provides": {
                "commands": [
                    {
                        "name": "pdca.alias-shortcut.cmd",
                        "file": "commands/cmd.md",
                        "aliases": ["pdca.shortcut"],
                    }
                ]
            },
        }
        ...
        assert manifest.commands[0]["aliases"] == ["pdca.shortcut"]
        assert manifest.warnings == []
    ...
    def test_install_rejects_namespace_squatting(self, temp_dir, project_dir):
        ...
        manifest_data = {
            ...
            "provides": {
                "commands": [
                    {
                        "name": "pdca.other-ext.cmd",
                        "file": "commands/cmd.md",
                        "aliases": ["pdca.squat-ext.ok"],
                    }
                ]
            },
        }
    ...
    def test_install_rejects_command_collision_with_installed_extension(self, temp_dir, project_dir):
        ...
        first_manifest = {
            ...
            "provides": {
                "commands": [
                    {
                        "name": "pdca.ext-one.sync",
                        "file": "commands/cmd.md",
                        "aliases": ["pdca.shared.sync"],
                    }
                ]
            },
        }
        ...
        installed_ext_dir = project_dir / ".pdca" / "extensions" / "ext-one"
    ...
    def test_config_backup_on_remove(self, extension_dir, project_dir):
        ...
        backup_dir = project_dir / ".pdca" / "extensions" / ".backup" / "test-ext"
...
"""
    ...
    def test_adjust_script_paths_does_not_mutate_input(self):
        ...
        from pdca_cli.agents import CommandRegistrar as AgentCommandRegistrar
        ...
        assert adjusted["scripts"]["sh"] == ".pdca/scripts/bash/setup-plan.sh {ARGS}"
        assert adjusted["scripts"]["ps"] == ".pdca/scripts/powershell/setup-plan.ps1 {ARGS}"

    def test_adjust_script_paths_preserves_extension_local_paths(self):
        """Extension-local script paths should not be rewritten into .pdca/.specify."""
        from pdca_cli.agents import CommandRegistrar as AgentCommandRegistrar
        registrar = AgentCommandRegistrar()
        original = {
            "scripts": {
                "sh": ".pdca/extensions/test-ext/scripts/setup.sh {ARGS}",
                "ps": "scripts/powershell/setup-plan.ps1 {ARGS}",
            }
        }
        ...
        assert adjusted["scripts"]["sh"] == ".pdca/extensions/test-ext/scripts/setup.sh {ARGS}"
        assert adjusted["scripts"]["ps"] == ".pdca/scripts/powershell/setup-plan.ps1 {ARGS}"
    ...
    def test_rewrite_project_relative_paths_preserves_extension_local_body_paths(self):
        ...
        body = (
            "Read `.pdca/extensions/test-ext/templates/spec.md`\n"
            "Run scripts/bash/setup-plan.sh\n"
        )

        rewritten = AgentCommandRegistrar.rewrite_project_relative_paths(body)

        assert ".pdca/extensions/test-ext/templates/spec.md" in rewritten
        assert ".pdca/scripts/bash/setup-plan.sh" in rewritten
    ...
    def test_register_commands_for_claude(self, extension_dir, project_dir):
        ...
        assert "pdca.test-ext.hello" in registered

        # Check command file was created
        cmd_file = claude_dir / "pdca-test-ext-hello" / "SKILL.md"
    ...
    def test_command_with_aliases(self, project_dir, temp_dir):
        ...
        manifest_data = {
            ...
            "provides": {
                "commands": [
                    {
                        "name": "pdca.ext-alias.cmd",
                        "file": "commands/cmd.md",
                        "aliases": ["pdca.ext-alias.shortcut"],
                    }
                ]
            },
        }
        ...
        assert "pdca.ext-alias.cmd" in registered
        assert "pdca.ext-alias.shortcut" in registered
        assert (claude_dir / "pdca-ext-alias-cmd" / "SKILL.md").exists()
        assert (claude_dir / "pdca-ext-alias-shortcut" / "SKILL.md").exists()
    ...
    def test_unregister_commands_handles_legacy_dot_notated_files(self, project_dir):
        ...
        # Command name: pdca.git.commit
        # Formatted name: pdca-git-commit
        cmd_name = "pdca.git.commit"
        formatted_name = "pdca-git-commit"
    ...
    def test_codex_skill_registration_writes_skill_frontmatter(self, extension_dir, project_dir):
        ...
        skill_file = skills_dir / "pdca-test-ext-hello" / "SKILL.md"
        assert skill_file.exists()

        content = skill_file.read_text()
        assert "name: pdca-test-ext-hello" in content
        assert "description: Test hello command" in content
...
"""
        ...
        init_options = project_dir / ".pdca" / "init-options.json"
        ...
        skill_file = skills_dir / "pdca-ext-scripted-plan" / "SKILL.md"
        assert skill_file.exists()
        ...
        assert '.pdca/scripts/bash/setup-plan.sh --json "$ARGUMENTS"' in content
    ...
    def test_all_skill_agents_register_commands_with_resolved_placeholders(
        ...
        manifest_data = {
            ...
            "provides": {
                "commands": [
                    {
                        "name": f"pdca.ext-{agent_name}.run",
        ...
        skill_dir_name = f"pdca-ext-{agent_name}-run"
        ...
        assert '.pdca/scripts/bash/setup-plan.sh' in content
    ...
    def test_codex_skill_alias_frontmatter_matches_alias_name(self, project_dir, temp_dir):
        ...
        manifest_data = {
            ...
            "provides": {
                "commands": [
                    {
                        "name": "pdca.ext-alias-skill.cmd",
                        "file": "commands/cmd.md",
                        "aliases": ["pdca.ext-alias-skill.shortcut"],
                    }
                ]
            },
        }
        ...
        primary = skills_dir / "pdca-ext-alias-skill-cmd" / "SKILL.md"
        alias = skills_dir / "pdca-ext-alias-skill-shortcut" / "SKILL.md"

        assert primary.exists()
        assert alias.exists()
        assert "name: pdca-ext-alias-skill-cmd" in primary.read_text()
        assert "name: pdca-ext-alias-skill-shortcut" in alias.read_text()
    ...
    def test_codex_skill_registration_uses_fallback_script_variant_without_init_options(
        ...
        manifest_data = {
            ...
            "provides": {
                "commands": [
                    {
                        "name": "pdca.ext-script-fallback.plan",
                        "file": "commands/plan.md",
                    }
                ]
            },
...
"""
        ...
        # Intentionally do NOT create .pdca/init-options.json
        skills_dir = project_dir / ".agents" / "skills"
        skills_dir.mkdir(parents=True)
        ...
        skill_file = skills_dir / "pdca-ext-script-fallback-plan" / "SKILL.md"
        assert skill_file.exists()

        content = skill_file.read_text()
        assert "{SCRIPT}" not in content
        if platform.system().lower().startswith("win"):
            assert ".pdca/scripts/powershell/setup-plan.ps1 -Json" in content
        else:
            assert '.pdca/scripts/bash/setup-plan.sh --json "$ARGUMENTS"' in content
    ...
    def test_codex_skill_registration_handles_non_dict_init_options(
        ...
        manifest_data = {
            ...
            "provides": {
                "commands": [
                    {
                        "name": "pdca.ext-script-list-init.plan",
                        "file": "commands/plan.md",
                    }
                ]
            },
...
"""
        ...
        content = (skills_dir / "pdca-ext-script-list-init-plan" / "SKILL.md").read_text()
        assert '.pdca/scripts/bash/setup-plan.sh --json "$ARGUMENTS"' in content
    ...
    def test_codex_skill_registration_fallback_prefers_powershell_on_windows(
        ...
        monkeypatch.setattr("pdca_cli.agents.platform.system", lambda: "Windows")
        ...
        manifest_data = {
            ...
            "provides": {
                "commands": [
                    {
                        "name": "pdca.ext-script-windows-fallback.plan",
                        "file": "commands/plan.md",
                    }
                ]
            },
...
"""
        ...
        skill_file = skills_dir / "pdca-ext-script-windows-fallback-plan" / "SKILL.md"
        assert skill_file.exists()

        content = skill_file.read_text()
        assert ".pdca/scripts/powershell/setup-plan.ps1 -Json" in content
        assert ".pdca/scripts/bash/setup-plan.sh" not in content
    ...
    def test_register_commands_for_copilot(self, extension_dir, project_dir):
        ...
        cmd_file = agents_dir / "pdca.test-ext.hello.agent.md"
        assert cmd_file.exists()

        # Verify NO plain .md file was created
        plain_md_file = agents_dir / "pdca.test-ext.hello.md"
    ...
    def test_dev_register_commands_symlinks_rendered_copilot_agent(
        ...
        assert registered == ["pdca.test-ext.hello"]

        cmd_file = agents_dir / "pdca.test-ext.hello.agent.md"
        assert cmd_file.is_symlink()

        target = cmd_file.resolve()
        assert ".pdca-dev" in target.parts
    ...
    def test_dev_register_commands_falls_back_to_copy_when_symlink_fails(
        ...
        monkeypatch.setattr("pdca_cli.agents.os.symlink", raise_symlink_error)
    ...
    def test_dev_register_commands_falls_back_to_copy_when_relpath_fails(
        ...
        monkeypatch.setattr("pdca_cli.agents.os.path.relpath", raise_relpath_error)
    ...
    def test_copilot_companion_prompt_created(self, extension_dir, project_dir):
        ...
        prompt_file = project_dir / ".github" / "prompts" / "pdca.test-ext.hello.prompt.md"
        assert prompt_file.exists()

        # Verify content has correct agent frontmatter
        content = prompt_file.read_text()
        assert content == "---\nagent: pdca.test-ext.hello\n---\n"
    ...
    def test_copilot_aliases_get_companion_prompts(self, project_dir, temp_dir):
        ...
        manifest_data = {
            ...
            "provides": {
                "commands": [
                    {
                        "name": "pdca.ext-alias-copilot.cmd",
                        "file": "commands/cmd.md",
                        "aliases": ["pdca.ext-alias-copilot.shortcut"],
                    }
                ]
            },
        }
        ...
        assert (prompts_dir / "pdca.ext-alias-copilot.cmd.prompt.md").exists()
        assert (prompts_dir / "pdca.ext-alias-copilot.shortcut.prompt.md").exists()
    ...
    def test_unregister_skill_removes_parent_directory(self, project_dir, temp_dir):
        ...
        registrar = CommandRegistrar()
        from pdca_cli.extensions import ExtensionManifest
        manifest = ExtensionManifest(ext_dir / "extension.yml")
        registrar.register_commands_for_agent("codex", manifest, ext_dir, project_dir)

        skill_subdir = skills_dir / "pdca-cleanup-ext-run"
        assert skill_subdir.exists(), "Skill subdirectory should exist after registration"
        assert (skill_subdir / "SKILL.md").exists()

        registrar.unregister_commands({"codex": ["pdca.cleanup-ext.run"]}, project_dir)
...
class TestIntegration:
    ...
    def test_full_install_and_remove_workflow(self, extension_dir, project_dir):
        ...
        cmd_file = project_dir / ".claude" / "skills" / "pdca-test-ext-hello" / "SKILL.md"
        assert cmd_file.exists()
    ...
    def test_copilot_cleanup_removes_prompt_files(self, extension_dir, project_dir):
        ...
        agent_file = agents_dir / "pdca.test-ext.hello.agent.md"
        prompt_file = project_dir / ".github" / "prompts" / "pdca.test-ext.hello.prompt.md"
...
class TestExtensionCatalog:
    ...
    def test_catalog_initialization(self, temp_dir):
        ...
        project_dir.mkdir()
        (project_dir / ".pdca").mkdir()

        catalog = ExtensionCatalog(project_dir)

        assert catalog.project_root == project_dir
        assert catalog.cache_dir == project_dir / ".pdca" / "extensions" / ".cache"
    ...
    def test_search_all_extensions(self, temp_dir):
        ...
        config_path = project_dir / ".pdca" / "extension-catalogs.yml"
    ...
    def test_fetch_single_catalog_sends_auth_header(self, temp_dir, monkeypatch):
        ...
        with patch("pdca_cli.authentication.http.urllib.request.build_opener", return_value=mock_opener):
    ...
    def test_download_extension_sends_auth_header(self, temp_dir, monkeypatch):
        ...
        with patch.object(catalog, "get_extension_info", return_value=ext_info), \
             patch("pdca_cli.authentication.http.urllib.request.build_opener", return_value=mock_opener):
...
class TestCatalogStack:
    ...
    def _make_project(self, temp_dir: Path) -> Path:
        """Create a minimal pdca-kit project directory."""
        project_dir = temp_dir / "project"
        project_dir.mkdir()
        (project_dir / ".pdca").mkdir()
        return project_dir
    ...
    def _write_catalog_config(self, project_dir: Path, catalogs: list) -> None:
        """Write extension-catalogs.yml to project .pdca dir."""
        import yaml as yaml_module
    ...
    def test_env_var_overrides_default_stack(self, temp_dir, monkeypatch):
        """PDCA_CATALOG_URL replaces the entire default stack."""
        project_dir = self._make_project(temp_dir)
        custom_url = "https://example.com/catalog.json"
        monkeypatch.setenv("PDCA_CATALOG_URL", custom_url)
    ...
    def test_env_var_invalid_url_raises(self, temp_dir, monkeypatch):
        """PDCA_CATALOG_URL with http:// (non-localhost) raises ValidationError."""
        project_dir = self._make_project(temp_dir)
        monkeypatch.setenv("PDCA_CATALOG_URL", "http://example.com/catalog.json")
    ...
    def test_load_catalog_config_missing_file(self, temp_dir):
        ...
        result = catalog._load_catalog_config(project_dir / ".pdca" / "nonexistent.yml")
...
class TestExtensionIgnore:
    ...
    def test_no_extensionignore(self, temp_dir, valid_manifest_data):
        ...
        dest = proj_dir / ".pdca" / "extensions" / "test-ext"
...
class TestExtensionAddCLI:
    ...
    def test_add_dev_links_copilot_agent_when_supported(
        ...
        if can_create_symlink(temp_dir):
            ...
            assert ".pdca-dev" in agent_file.resolve().parts
        else:
    ...
    def test_add_dev_falls_back_to_copy_when_windows_symlinks_unavailable(
        ...
        monkeypatch.setattr(
            "pdca_cli.agents.os.symlink", raise_windows_symlink_error
        )
    ...
    def test_add_by_display_name_uses_resolved_id_for_download(self, tmp_path):
        ...
        project_dir.mkdir()
        (project_dir / ".pdca").mkdir()
        (project_dir / ".pdca" / "extensions").mkdir(parents=True)
        ...
        with patch("pdca_cli.extensions.ExtensionCatalog", return_value=mock_catalog), \
    ...
    def test_add_bundled_extension_not_found_gives_clear_error(self, tmp_path):
        ...
        with patch("pdca_cli.extensions.ExtensionCatalog", return_value=mock_catalog), \
             patch("pdca_cli._locate_bundled_extension", return_value=None), \
             patch.object(Path, "cwd", return_value=project_dir):
            result = runner.invoke(
                app,
                ["extension", "add", "git"],
        ...
        assert result.exit_code != 0
        assert "bundled with pdca-kit" in result.output
    ...
    def test_add_from_url_prompts_before_spinner(self, tmp_path):
        ...
        runner = CliRunner()
        with patch.object(Path, "cwd", return_value=project_dir), \
             patch("pdca_cli.console.status", side_effect=record_status), \
...
class TestDownloadExtensionBundled:
    ...
    def test_download_extension_raises_for_bundled(self, temp_dir):
        ...
        with patch.object(catalog, "get_extension_info", return_value=bundled_ext_info):
            with pytest.raises(ExtensionError, match="bundled with pdca-kit"):
...
class TestExtensionUpdateCLI:
    ...
    def _create_catalog_zip(zip_path: Path, version: str):
        ...
        manifest = {
            ...
            "requires": {"pdca_version": ">=0.1.0"},
            "provides": {"commands": [{"name": "pdca.test-ext.hello", "file": "commands/hello.md"}]},
        }
    ...
    def test_update_success_preserves_installed_at(self, tmp_path):
        ...
        original_config_content = (
            project_dir / ".pdca" / "extensions" / "test-ext" / "linear-config.yml"
        ).read_text()
        ...
        def fake_install_from_zip(self_obj, _zip_path, pdca_version):
            return self_obj.install_from_directory(v2_dir, pdca_version)
    ...
    def test_update_failure_rolls_back_registry_hooks_and_commands(self, tmp_path, monkeypatch):
        ...
        hooks_before = yaml.safe_load((project_dir / ".pdca" / "extensions.yml").read_text())

        registered_commands = backup_registry_entry.get("registered_commands", {})
        command_files = []
        from pdca_cli.agents import CommandRegistrar as AgentRegistrar
        ...
        hooks_after = yaml.safe_load((project_dir / ".pdca" / "extensions.yml").read_text())
...
class TestExtensionPriority:
    ...
    def test_corrupted_extension_entry_not_picked_up_as_unregistered(self, project_dir):
        ...
        extensions_dir = project_dir / ".pdca" / "extensions"
        ...
        from pdca_cli.presets import PresetResolver
...
class TestExtensionPriorityCLI:
    ...
    def test_set_priority_not_installed(self, project_dir):
        ...
        # Ensure .pdca exists
        (project_dir / ".pdca").mkdir(parents=True, exist_ok=True)
...
class TestHookInvocationRendering:
    ...
    def test_kimi_hooks_render_skill_invocation(self, project_dir):
        """Kimi projects should render /skill:pdca-* invocations."""
        init_options = project_dir / ".pdca" / "init-options.json"
        init_options.parent.mkdir(parents=True, exist_ok=True)
        ...
        assert "Executing: `/skill:pdca-plan`" in message
        assert "EXECUTE_COMMAND: pdca.plan" in message
        assert "EXECUTE_COMMAND_INVOCATION: /skill:pdca-plan" in message

    def test_codex_hooks_render_dollar_skill_invocation(self, project_dir):
        """Codex projects with --ai-skills should render $pdca-* invocations."""
        init_options = project_dir / ".pdca" / "init-options.json"
        init_options.parent.mkdir(parents=True, exist_ok=True)
        ...
        assert execution["command"] == "pdca.tasks"
        assert execution["invocation"] == "$pdca-tasks"

    def test_cline_hooks_render_hyphenated_invocation(self, project_dir):
        """Cline projects should render /pdca-* invocations."""
        init_options = project_dir / ".pdca" / "init-options.json"
        init_options.parent.mkdir(parents=True, exist_ok=True)
    ...
    def test_cline_hooks_render_extension_command(self, project_dir):
        """Cline projects should render /pdca-my-ext-cmd for extension hooks."""
        init_options = project_dir / ".pdca" / "init-options.json"
        init_options.parent.mkdir(parents=True, exist_ok=True)
        init_options.write_text(json.dumps({"ai": "cline"}))

        hook_executor = HookExecutor(project_dir)
        # Test with a non-pdca. command
        execution = hook_executor.execute_hook(
            {
                "extension": "test-ext",
        ...
        assert execution["invocation"] == "/pdca-my-extension-do-something"
    ...
    def test_extension_command_uses_hyphenated_skill_invocation(self, project_dir):
        ...
        assert "Executing: `/skill:pdca-test-ext-hello`" in message
        assert "EXECUTE_COMMAND: pdca.test-ext.hello" in message
        assert "EXECUTE_COMMAND_INVOCATION: /skill:pdca-test-ext-hello" in message
    ...
    def test_hook_executor_caches_init_options_lookup(self, project_dir, monkeypatch):
        ...
        monkeypatch.setattr("pdca_cli.load_init_options", fake_load_init_options)

        hook_executor = HookExecutor(project_dir)
        assert hook_executor._render_hook_invocation("pdca.plan") == "/skill:pdca-plan"
        assert hook_executor._render_hook_invocation("pdca.tasks") == "/skill:pdca-tasks"
...
class TestExtensionRemoveCLI:
    ...
    def test_remove_confirmation_singular_command(self, tmp_path, extension_dir):
        ...
        manager.registry.update("test-ext", {"registered_commands": {"claude": ["pdca.test-ext.hello"]}})
    ...
    def test_remove_confirmation_plural_commands(self, tmp_path, extension_dir):
        ...
        manager.registry.update("test-ext", {"registered_commands": {"claude": ["pdca.test-ext.hello", "pdca.test-ext.run"]}})
    ...
    def _setup_mock_extension(self, tmp_path, ai_name):
        ...
        manifest_data = {
            ...
            "provides": {
                "commands": [
                    {
                        "name": "pdca.mock-ext.hello",
                        "file": "commands/hello.md",
                        "description": "Test hello command",
                        "aliases": ["pdca.mock-ext.greet"]
                    }
                ]
            }
        }
        ...
        # Command file with dotted pdca references in frontmatter and body
        cmd_content = """---
description: "Test hello command"
agent: pdca.tasks
handoffs:
  - agent: pdca.iterate.start
    message: "Hand off to start"
---

# Test Hello Command

Please refer to pdca.mock-ext.greet for instructions.
$ARGUMENTS
"""
    ...
    def test_cline_extension_hyphenation(self, tmp_path):
        ...
        # Note: We assert that the primary command 'pdca-mock-ext-hello' is printed,
        # but we do not assert that the alias 'pdca-mock-ext-greet' is printed in the console
        # because manifest.commands only lists primary commands.
        assert "pdca-mock-ext-hello" in result.output
        assert "pdca.mock-ext.hello" not in result.output

        # Verify on-disk command names are hyphenated
        hello_file = cline_workflows_dir / "pdca-mock-ext-hello.md"
        greet_file = cline_workflows_dir / "pdca-mock-ext-greet.md"
        ...
        assert hello_fm["agent"] == "pdca-tasks"
        assert hello_fm["handoffs"][0]["agent"] == "pdca-iterate-start"

        # Verify body references are hyphenated for Cline
        assert "pdca-mock-ext-greet" in hello_body
        assert "pdca.mock-ext.greet" not in hello_body
    ...
    def test_non_cline_extension_no_hyphenation(self, tmp_path):
        ...
        hello_file = claude_commands_dir / "pdca.mock-ext.hello.md"
        greet_file = claude_commands_dir / "pdca.mock-ext.greet.md"
