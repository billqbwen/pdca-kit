...
from pdca_cli import AGENT_CONFIG, AI_ASSISTANT_ALIASES, AI_ASSISTANT_HELP
...
class TestAgentConfigConsistency:
    ...
    def test_skills_agents_have_hyphen_invoke_separator_in_agent_configs(self):
        ...
        ``__PDCA_COMMAND_*__`` tokens with the correct hyphen separator.
        """
        cfg = CommandRegistrar.AGENT_CONFIGS
        skills_agents = [
    ...
    def test_skills_agent_command_token_resolves_with_hyphen(self, tmp_path):
        """__PDCA_COMMAND_*__ tokens in extension commands resolve to /pdca-<cmd>
        ...
        register_commands() fell back to '.' and emitted /pdca.pdca instead
        of /pdca-specify for skills agents.
        """
        import re
        from pathlib import Path
        ...
        commands = [
            {"name": "pdca.git.feature", "file": "commands/pdca.git.feature.md"}
        ]
        ...
        skill_file = (
            tmp_path / ".claude" / "skills" / "pdca-git-feature" / "SKILL.md"
        )
        ...
        assert not re.search(r"(?<![a-zA-Z0-9_])/pdca\.[a-z]", content), (
            "Found dot-notation command ref (/pdca.<cmd>) in generated Claude skill. "
