...
class TestVersionFlag:
    ...
    def test_version_long_flag(self):
        """pdca --version prints version and exits 0."""
        with patch("pdca_cli.get_pdca_version", return_value="1.2.3"):
            result = runner.invoke(app, ["--version"])
        assert result.exit_code == 0
        assert "pdca 1.2.3" in result.output

    def test_version_short_flag(self):
        """pdca -V prints version and exits 0."""
        with patch("pdca_cli.get_pdca_version", return_value="1.2.3"):
    ...
    def test_version_flag_takes_precedence_over_subcommand(self):
        ...
        with patch("pdca_cli.get_pdca_version", return_value="0.7.2"):
            result = runner.invoke(app, ["--version", "init"])
        assert result.exit_code == 0
        assert "pdca 0.7.2" in result.output
...
class TestVersionCommand:
    ...
    def test_version_features_text(self):
        ...
        assert result.exit_code == 0
        assert "PDCA Kit CLI: 1.2.3" in result.output
