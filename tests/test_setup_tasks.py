...
def _install_ps_scripts(repo: Path) -> None:
    d = repo / ".pdca" / "scripts" / "powershell"
    d.mkdir(parents=True, exist_ok=True)
...
def test_setup_tasks_bash_extension_wins_over_core(tasks_repo: Path) -> None:
    ...
    extension_dir = (
        tasks_repo / ".pdca" / "extensions" / "test-extension" / "templates"
    )
...
def test_setup_tasks_bash_preset_wins_over_extension(tasks_repo: Path) -> None:
    ...
    # FIX: real preset layout is .pdca/presets/<id>/templates/<name>.md
    preset_dir = tasks_repo / ".pdca" / "presets" / "test-preset" / "templates"
...
def test_setup_tasks_bash_preset_priority_order(tasks_repo: Path) -> None:
    ...
    high_priority_dir = (
        tasks_repo / ".pdca" / "presets" / "priority-1-preset" / "templates"
    )
    ...
    low_priority_dir = (
        tasks_repo / ".pdca" / "presets" / "priority-2-preset" / "templates"
    )
    ...
    registry_json = tasks_repo / ".pdca" / "presets" / ".registry"
...
def test_setup_tasks_bash_passes_custom_branch_when_feature_json_valid(
    ...
    (tasks_repo / ".pdca" / "feature.json").write_text(
...
def test_setup_tasks_ps_core_template_resolved(tasks_repo: Path) -> None:
    ...
    TASKS_TEMPLATE path.
    """
    script = tasks_repo / ".pdca" / "scripts" / "powershell" / "setup-tasks.ps1"
