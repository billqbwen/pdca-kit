...
def _clean_env() -> dict[str, str]:
    ...
    setup-plan.{sh,ps1} honors PDCA_FEATURE, PDCA_FEATURE_DIRECTORY, etc.,
    ...
    .pdca/feature.json state set up by the fixture.
    """
    env = os.environ.copy()
    for key in list(env):
...
def test_setup_plan_passes_custom_branch_when_feature_json_valid(plan_repo: Path) -> None:
    ...
    (plan_repo / ".pdca" / "feature.json").write_text(
        json.dumps({"feature_directory": "specs/001-tiny-notes-app"}),
        encoding="utf-8",
    )
    script = plan_repo / ".pdca" / "scripts" / "bash" / "setup-plan.sh"
...
def test_setup_plan_ps_passes_custom_branch_when_feature_json_valid(plan_repo: Path) -> None:
    ...
    script = plan_repo / ".pdca" / "scripts" / "powershell" / "setup-plan.ps1"
