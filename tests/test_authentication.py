...
from pdca_cli.authentication import AUTH_REGISTRY, _register, get_provider
from pdca_cli.authentication.azure_devops import AzureDevOpsAuth
from pdca_cli.authentication.base import AuthProvider
from pdca_cli.authentication.config import (
    AuthConfigEntry,
    find_entries_for_url,
    load_auth_config,
)
from pdca_cli.authentication.github import GitHubAuth
...
class TestAzureDevOpsAuth:
    ...
    def test_resolve_token_azure_cli_success(self):
        ...
        with patch("pdca_cli.authentication.azure_devops.subprocess.run", return_value=result):
    ...
    def test_resolve_token_azure_cli_not_installed_returns_none(self):
        ...
        with patch("pdca_cli.authentication.azure_devops.subprocess.run", side_effect=OSError("not found")):
            assert AzureDevOpsAuth().resolve_token(entry) is None
...
class TestAuthenticatedHttp:
    ...
    def test_open_url_no_auth_for_non_matching_host(self, monkeypatch):
        ...
        def fake_urlopen(req, timeout=None):
            ...
            resp = MagicMock(); resp.__enter__ = lambda s: s; resp.__exit__ = MagicMock(return_value=False)
            return resp
        with patch("pdca_cli.authentication.http.urllib.request.urlopen", side_effect=fake_urlopen):
    ...
    def test_open_url_falls_through_on_401(self, monkeypatch):
        ...
        with patch("pdca_cli.authentication.http.urllib.request.build_opener", return_value=mock_opener), \
             patch("pdca_cli.authentication.http.urllib.request.urlopen", side_effect=fake_side_effect):
            open_url("https://github.com/org/repo")
        assert call_count == 2
...
class TestAuthenticatedHttpNegative:
    ...
    def test_urlerror_propagates(self, monkeypatch):
        ...
        from pdca_cli.authentication.http import open_url
        self._set_config(monkeypatch, [])
        with patch("pdca_cli.authentication.http.urllib.request.urlopen",
...
class TestRedirectStripping:
    def test_redirect_within_hosts_preserves_auth(self):
        from pdca_cli.authentication.http import _StripAuthOnRedirect
...
class TestFetchLatestReleaseTagDelegation:
    ...
    def test_gh_token_forwarded_when_configured(self, monkeypatch):
        from unittest.mock import MagicMock, patch
        from pdca_cli._version import _fetch_latest_release_tag
    ...
    def test_no_config_means_no_auth(self, monkeypatch):
        ...
        self._set_config(monkeypatch, [])
        captured, side_effect = self._capture_request()
        with patch("pdca_cli.authentication.http.urllib.request.urlopen", side_effect=side_effect):
            _fetch_latest_release_tag()
