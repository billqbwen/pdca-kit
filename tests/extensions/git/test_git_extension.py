...
def _setup_project(tmp_path: Path, *, git: bool = True) -> Path:
    ...
    ext_bash = tmp_path / ".pdca" / "extensions" / "git" / "scripts" / "bash"
    ...
    # Copy extension.yml
    shutil.copy(EXT_DIR / "extension.yml", tmp_path / ".pdca" / "extensions" / "git" / "extension.yml")
...
def _write_config(project: Path, content: str) -> Path:
    ...
    config_path = project / ".pdca" / "extensions" / "git" / "git-config.yml"
...
def _run_bash(script_name: str, cwd: Path, *args: str, env_extra: dict | None = None) -> subprocess.CompletedProcess:
    ...
    script = cwd / ".pdca" / "extensions" / "git" / "scripts" / "bash" / script_name
...
def _run_pwsh(script_name: str, cwd: Path, *args: str) -> subprocess.CompletedProcess:
    ...
    script = cwd / ".pdca" / "extensions" / "git" / "scripts" / "powershell" / script_name
...
class TestGitExtensionManifest:
    ...
    def test_manifest_commands(self):
        ...
        assert "pdca.git.feature" in names
        assert "pdca.git.validate" in names
        assert "pdca.git.remote" in names
        assert "pdca.git.initialize" in names
        assert "pdca.git.commit" in names
...
class TestGitExtensionInstall:
    ...
    def test_install_copies_scripts(self, tmp_path: Path):
        ...
        ext_installed = tmp_path / ".pdca" / "extensions" / "git"
    ...
    def test_bundled_extension_locator(self):
        ...
        from pdca_cli import _locate_bundled_extension
...
class TestAutoCommitBash:
    ...
    def test_no_config_file_skips(self, tmp_path: Path):
        ...
        config = project / ".pdca" / "extensions" / "git" / "git-config.yml"
...
class TestGitCommonBash:
    def test_has_git_true(self, tmp_path: Path):
        ...
        project = _setup_project(tmp_path, git=True)
        script = project / ".pdca" / "extensions" / "git" / "scripts" / "bash" / "git-common.sh"
...
class TestGitCommonPowerShell:
    def test_test_feature_branch_accepts_single_prefix(self, tmp_path: Path):
        project = _setup_project(tmp_path)
        script = project / ".pdca" / "extensions" / "git" / "scripts" / "powershell" / "git-common.ps1"
        result = subprocess.run(
            [
                "pwsh",
                "-NoProfile",
