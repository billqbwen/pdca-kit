...
from pdca_cli.integrations.base import MarkdownIntegration
...
class TestGenericIntegration:
    ...
    def test_plan_references_correct_context_file(self, tmp_path):
        ...
        m = IntegrationManifest("generic", tmp_path)
        i.setup(tmp_path, m, parsed_options={"commands_dir": ".custom/cmds"})
        plan_file = tmp_path / ".custom" / "cmds" / "pdca.plan.md"
    ...
    def test_implement_loads_constitution_context(self, tmp_path):
        ...
        m = IntegrationManifest("generic", tmp_path)
        i.setup(tmp_path, m, parsed_options={"commands_dir": ".custom/cmds"})
        implement_file = tmp_path / ".custom" / "cmds" / "pdca.implement.md"
    ...
    def test_command_loads_constitution_context(self, tmp_path, command_stem):
        ...
        m = IntegrationManifest("generic", tmp_path)
        i.setup(tmp_path, m, parsed_options={"commands_dir": ".custom/cmds"})
        cmd_file = tmp_path / ".custom" / "cmds" / f"pdca.{command_stem}.md"
        assert cmd_file.exists(), f"Command file missing: {cmd_file.name}"
        content = cmd_file.read_text(encoding="utf-8")
        assert "constitution.md" in content, (
            f"pdca.{command_stem}.md must reference constitution.md"
        )
    ...
    def test_constitution_command_exists(self, tmp_path):
        ...
        m = IntegrationManifest("generic", tmp_path)
        i.setup(tmp_path, m, parsed_options={"commands_dir": ".custom/cmds"})
        cmd_file = tmp_path / ".custom" / "cmds" / "pdca.constitution.md"
    ...
    def test_complete_file_inventory_sh(self, tmp_path):
        ...
        expected = sorted([
            "AGENTS.md",
            ".myagent/commands/pdca.analyze.md",
