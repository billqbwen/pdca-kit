...
def _init_project(tmp_path, integration="copilot"):
    """Helper: init a pdca-kit project with the given integration."""
...
def _write_invalid_manifest(project, key):
    manifest = project / ".pdca" / "integrations" / f"{key}.manifest.json"
...
class TestIntegrationList:
    def test_list_requires_pdca_project(self, tmp_path):
        old_cwd = os.getcwd()
        try:
            os.chdir(tmp_path)
    ...
    def test_list_rejects_newer_integration_state_schema(self, tmp_path):
        project = _init_project(tmp_path, "claude")
        int_json = project / ".pdca" / "integration.json"
...
class TestIntegrationInstall:
    def test_install_requires_pdca_project(self, tmp_path):
        old_cwd = os.getcwd()
        try:
            os.chdir(tmp_path)
    ...
    def test_install_multi_safe_integration(self, tmp_path):
        ...
        assert (project / ".agents" / "skills" / "pdca-plan" / "SKILL.md").exists()

    def test_install_non_default_refreshes_init_options_version_only(self, tmp_path, monkeypatch):
        project = _init_project(tmp_path, "claude")
        init_options = project / ".pdca" / "init-options.json"
        opts = json.loads(init_options.read_text(encoding="utf-8"))
        opts["pdca_version"] = "0.6.1"
        init_options.write_text(json.dumps(opts), encoding="utf-8")

        import pdca_cli.integrations._commands as _int_cmds

        monkeypatch.setattr(_int_cmds, "get_pdca_version", lambda: "0.8.11")
        ...
        assert updated["pdca_version"] == "0.8.11"
    ...
    def test_install_into_bare_project(self, tmp_path):
        """Install into a project with .pdca/ but no integration."""
    ...
    def test_install_bare_project_gets_shared_infra(self, tmp_path):
        ...
        assert (project / ".pdca" / "scripts").is_dir()
...
class TestIntegrationUninstall:
    def test_uninstall_requires_pdca_project(self, tmp_path):
        old_cwd = os.getcwd()
        try:
            os.chdir(tmp_path)
    ...
    def test_uninstall_removes_files(self, tmp_path):
        ...
        assert not (project / ".claude" / "skills" / "pdca-plan" / "SKILL.md").exists()

        # Manifest removed
        assert not (project / ".pdca" / "integrations" / "claude.manifest.json").exists()

        # integration.json removed
        assert not (project / ".pdca" / "integration.json").exists()

    def test_uninstall_preserves_modified_files(self, tmp_path):
        """Full lifecycle: install → modify → uninstall → modified file kept."""
        project = _init_project(tmp_path, "claude")
        plan_file = project / ".claude" / "skills" / "pdca-plan" / "SKILL.md"
        ...
        assert ".claude/skills/pdca-plan/SKILL.md" in result.output
    ...
    def test_uninstall_non_default_preserves_default(self, tmp_path):
        ...
        try:
            ...
            os.chdir(old_cwd)
        assert result.exit_code == 0, result.output
        assert not (project / ".agents" / "skills" / "pdca-plan" / "SKILL.md").exists()
        assert (project / ".claude" / "skills" / "pdca-plan" / "SKILL.md").exists()
    ...
    def test_uninstall_default_refreshes_templates_for_fallback(self, tmp_path):
        ...
        template = project / ".pdca" / "templates" / "plan-template.md"
        script = project / ".pdca" / "scripts" / "bash" / "check-prerequisites.sh"
        assert "/pdca.plan" in template.read_text(encoding="utf-8")
        assert "/pdca.plan" in script.read_text(encoding="utf-8")
    ...
    def test_uninstall_preserves_shared_infra(self, tmp_path):
        ...
        project = _init_project(tmp_path, "claude")
        shared_script = project / ".pdca" / "scripts" / "bash" / "common.sh"
...
class TestIntegrationUse:
    ...
    def test_use_refreshes_shared_templates_between_command_styles(self, tmp_path):
        ...
        try:
            ...
            assert "/pdca.plan" in template.read_text(encoding="utf-8")
            assert "/pdca.plan" in script.read_text(encoding="utf-8")
            assert "/pdca-plan" not in script.read_text(encoding="utf-8")
    ...
    def test_use_preserves_modified_templates_unless_forced(self, tmp_path):
        project = _init_project(tmp_path, "claude")
        template = project / ".pdca" / "templates" / "plan-template.md"
        template.write_text("custom template with /pdca-plan\n", encoding="utf-8")
    ...
    def test_use_does_not_persist_default_when_shared_infra_refresh_fails(self, tmp_path, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setattr(pdca_cli, "_install_shared_infra", fail_refresh)
...
class TestIntegrationSwitch:
    def test_switch_requires_pdca_project(self, tmp_path):
        old_cwd = os.getcwd()
        try:
            os.chdir(tmp_path)
    ...
    def test_switch_migrates_extension_commands(self, tmp_path):
        ...
        kimi_git_feature = project / ".kimi" / "skills" / "pdca-git-feature" / "SKILL.md"
        ...
        opencode_git_feature = project / ".opencode" / "commands" / "pdca.git.feature.md"
        ...
        registry = json.loads(
            (project / ".pdca" / "extensions" / ".registry").read_text(encoding="utf-8")
        )
        ...
        claude_git_feature = project / ".claude" / "skills" / "pdca-git-feature" / "SKILL.md"
    ...
    def test_switch_migrates_copilot_skills_extension_commands(self, tmp_path):
        ...
        copilot_git_feature = project / ".github" / "skills" / "pdca-git-feature" / "SKILL.md"
        copilot_agent_file = project / ".github" / "agents" / "pdca.git.feature.agent.md"
        ...
        assert "pdca-git-feature" in git_meta["registered_skills"]
    ...
    def test_switch_refreshes_managed_shared_script_refs(self, tmp_path):
        ...
        assert "/pdca-plan" in shared_content
        ...
        assert "/pdca.plan" in updated
        assert "/pdca-plan" not in updated
    ...
    def test_switch_refreshes_stale_managed_shared_infra(self, tmp_path):
        ...
        project = _init_project(tmp_path, "claude")
        shared_script = project / ".pdca" / "scripts" / "bash" / "common.sh"
        assert "/pdca-plan" in shared_script.read_text(encoding="utf-8")

        # Simulate a stale vendored script: write truncated content as bytes
        # (write_text would translate \n→\r\n on Windows and break the hash)
        # and update the pdca.manifest hash so the stale copy is treated
        # as "managed" (installed by pdca-kit, not a user customization).
        stale_bytes = b"#!/usr/bin/env bash\n# stale vendored copy\n"
        shared_script.write_bytes(stale_bytes)

        manifest_path = project / ".pdca" / "integrations" / "pdca.manifest.json"
        manifest_data = json.loads(manifest_path.read_text(encoding="utf-8"))
        manifest_data["files"][".pdca/scripts/bash/common.sh"] = (
    ...
    def test_switch_skips_symlinked_parent_directory(self, tmp_path):
        """Regression: if .pdca/scripts/bash is a symlink, switch must not write through it.
        ...
        project = _init_project(tmp_path, "claude")
        bash_dir = project / ".pdca" / "scripts" / "bash"
...
class TestIntegrationUpgrade:
    ...
    def test_upgrade_refreshes_init_options_pdca_version(self, tmp_path, monkeypatch):
        project = _init_project(tmp_path, "claude")
        init_options = project / ".pdca" / "init-options.json"
        opts = json.loads(init_options.read_text(encoding="utf-8"))
        opts["pdca_version"] = "0.6.1"
    ...
    def test_upgrade_does_not_persist_state_when_shared_infra_refresh_fails(self, tmp_path, monkeypatch):
        ...
        int_json = project / ".pdca" / "integration.json"
        init_options = project / ".pdca" / "init-options.json"
        manifest_path = project / ".pdca" / "integrations" / "claude.manifest.json"
        ...
        real_install_shared_infra = pdca_cli._install_shared_infra
        calls = {"count": 0}
    ...
    def test_upgrade_default_refreshes_shared_script_refs_for_option_separator_change(self, tmp_path):
        project = _init_project(tmp_path, "copilot")
        template = project / ".pdca" / "templates" / "plan-template.md"
        managed_script = project / ".pdca" / "scripts" / "bash" / "check-prerequisites.sh"
    ...
    def test_upgrade_migrates_opencode_legacy_dir(self, tmp_path):
        ...
        manifest_path = project / ".pdca" / "integrations" / "opencode.manifest.json"
        manifest_data = json.loads(manifest_path.read_text(encoding="utf-8"))
        patched_files = {}
        ...
        old_commands = sorted(legacy.glob("pdca.*.md"))
        assert len(old_commands) > 0, "Legacy dir should have pdca command files"
        ...
        new_commands = sorted(canonical.glob("pdca.*.md"))
        ...
        core_remaining = [
            f for f in legacy.glob("pdca.*.md")
            if "agent-context" not in f.name
        ]
        assert len(core_remaining) == 0, (
            f"Legacy .opencode/command/ should have no core pdca files after upgrade, "
...
class TestParseIntegrationOptionsEqualsForm:
    def test_equals_form_parsed(self):
        ...
        from pdca_cli.integrations._commands import _parse_integration_options
...
class TestUninstallNoManifestClearsInitOptions:
    def test_init_options_cleared_on_no_manifest_uninstall(self, tmp_path):
        ...
        opts_json = project / ".pdca" / "init-options.json"
