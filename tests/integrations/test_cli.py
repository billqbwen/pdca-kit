...
class TestCliDiagnosticFormatting:
    def test_cli_error_detail_flattens_newlines(self):
        ...
        assert pdca_cli._cli_error_detail(RuntimeError("line one\nline two")) == "line one line two"

    def test_cli_error_detail_handles_empty_message(self):
        import pdca_cli

        assert pdca_cli._cli_error_detail(RuntimeError()) == "RuntimeError"

    def test_cli_phase_label_includes_target(self):
        import pdca_cli

        assert (
            pdca_cli._cli_phase_label("rollback", "integration", "codex")
...
class TestInitIntegrationFlag:
    ...
    def test_integration_copilot_creates_files(self, tmp_path):
        ...
        assert (project / ".github" / "prompts" / "pdca.plan.prompt.md").exists()
        assert (project / ".pdca" / "scripts" / "bash" / "common.sh").exists()

        data = json.loads((project / ".pdca" / "integration.json").read_text(encoding="utf-8"))
        assert data["integration"] == "copilot"

        opts = json.loads((project / ".pdca" / "init-options.json").read_text(encoding="utf-8"))
        assert opts["integration"] == "copilot"
        # context_file lives in the agent-context extension config, not init-options.json
        assert "context_file" not in opts

        import yaml as _yaml
        ext_cfg_path = project / ".pdca" / "extensions" / "agent-context" / "agent-context-config.yml"
        ...
        assert (project / ".pdca" / "integrations" / "copilot.manifest.json").exists()
    ...
    def test_noninteractive_init_defaults_to_copilot(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(pdca_cli, "select_with_arrows", fail_select)
        ...
        assert f"defaulting to '{pdca_cli.DEFAULT_INIT_INTEGRATION}'" in result.output
        assert (project / ".github" / "agents" / "pdca.plan.agent.md").exists()

        data = json.loads((project / ".pdca" / "integration.json").read_text(encoding="utf-8"))
        assert data["integration"] == pdca_cli.DEFAULT_INIT_INTEGRATION
    ...
    def test_ai_generic_warning_suggests_integration_options_equivalent(self, tmp_path):
        ...
        assert normalized_output.index("Deprecation Warning") < normalized_output.index("Next Steps")
        assert (project / ".myagent" / "commands" / "pdca.plan.md").exists()
    ...
    def test_shared_infra_skips_existing_files_without_force(self, tmp_path):
        ...
        from pdca_cli import _install_shared_infra
        ...
        scripts_dir = project / ".pdca" / "scripts" / "bash"
        ...
        templates_dir = project / ".pdca" / "templates"
    ...
    def test_shared_infra_warns_when_manifest_cannot_be_loaded(self, tmp_path, capsys):
        ...
        project.mkdir()
        integrations_dir = project / ".pdca" / "integrations"
        integrations_dir.mkdir(parents=True)
        manifest_path = integrations_dir / "pdca.manifest.json"
    ...
    def test_shared_template_refresh_refuses_symlinked_destination(self, tmp_path):
        ...
        from pdca_cli import _refresh_shared_templates
    ...
    def test_shared_infra_refuses_symlinked_specify_directory_before_mkdir(self, tmp_path):
        """Shared infra installs must not follow a symlinked .pdca directory."""
        from pdca_cli import _install_shared_infra
        ...
        outside.mkdir()
        os.symlink(outside, project / ".pdca")

        with pytest.raises(ValueError, match="symlinked"):
            _install_shared_infra(project, "sh", force=True)
        # Nothing should have been written under the symlinked .pdca target.
    ...
    def test_shared_infra_refuses_symlinked_shared_manifest(self, tmp_path):
        ...
        from pdca_cli.shared_infra import install_shared_infra
        ...
        outside.write_text("# outside\n", encoding="utf-8")
        os.symlink(outside, integrations_dir / "pdca.manifest.json")
    ...
    def test_shared_template_refresh_preflights_before_writing(self, tmp_path):
        ...
        from pdca_cli.shared_infra import refresh_shared_templates
    ...
    def test_shared_infra_install_supports_nested_script_sources(self, tmp_path):
        ...
        nested_dest = project / ".pdca" / "scripts" / "bash" / "nested" / "deep.sh"
    ...
    def test_shared_infra_skip_warning_uses_posix_paths(self, tmp_path):
        ...
        project.mkdir()
        nested_dest = project / ".pdca" / "scripts" / "bash" / "nested"
        nested_dest.mkdir(parents=True)
        (nested_dest / "deep.sh").write_text("# existing script\n", encoding="utf-8")

        templates_dest = project / ".pdca" / "templates"
        ...
        assert ".pdca/scripts/bash/nested/deep.sh" in output
        assert ".pdca/templates/plan-template.md" in output
    ...
    def test_shared_template_writes_are_not_world_writable(self, tmp_path):
        ...
        written = project / ".pdca" / "templates" / "plan-template.md"
...
class TestForceExistingDirectory:
    ...
    def test_force_merges_into_existing_dir(self, tmp_path):
        ...
        # PDCA Kit files should be installed
        assert (target / ".pdca" / "init-options.json").exists()
        assert (target / ".pdca" / "templates" / "spec-template.md").exists()
...
class TestGitExtensionAutoInstall:
    ...
    def test_git_extension_auto_installed(self, tmp_path):
        ...
        ext_dir = project / ".pdca" / "extensions" / "git"
        ...
        extensions_yml = project / ".pdca" / "extensions.yml"
    ...
    def test_git_extension_commands_registered(self, tmp_path):
        ...
        git_skills = [f for f in claude_skills.iterdir() if f.name.startswith("pdca-git-")]
        assert len(git_skills) > 0, "no git extension commands registered"


class TestSharedInfraCommandRefs:
    """Verify _install_shared_infra resolves __PDCA_COMMAND_*__ in shared infra."""
    ...
    def _combined_script_content(project, script_type):
        ...
        return "\n".join(
            (project / ".pdca" / "scripts" / script_dir / name).read_text(
                encoding="utf-8"
            )
            for name in names
        )
    ...
    def test_dot_separator_in_page_templates(self, tmp_path):
        """Markdown agents get /pdca.<name> in page templates."""
        from pdca_cli import _install_shared_infra
        ...
        plan = project / ".pdca" / "templates" / "plan-template.md"
        assert plan.exists()
        content = plan.read_text(encoding="utf-8")
        assert "__PDCA_COMMAND_" not in content, "unresolved placeholder in plan-template.md"
        assert "/pdca.plan" in content

        checklist = project / ".pdca" / "templates" / "checklist-template.md"
        content = checklist.read_text(encoding="utf-8")
        assert "__PDCA_COMMAND_" not in content
        assert "/pdca.checklist" in content

    def test_hyphen_separator_in_page_templates(self, tmp_path):
        """Skills agents get /pdca-<name> in page templates."""
        from pdca_cli import _install_shared_infra
        ...
        assert "__PDCA_COMMAND_" not in content, "unresolved placeholder in plan-template.md"
        assert "/pdca-plan" in content
        assert "/pdca.plan" not in content, "dot-notation leaked into skills page template"

        tasks = project / ".pdca" / "templates" / "tasks-template.md"
    ...
    def test_dot_separator_in_shared_scripts(self, tmp_path, script_type):
        """Markdown agents get /pdca.<name> in shared script hints."""
        from pdca_cli import _install_shared_infra
    ...
    def test_hyphen_separator_in_shared_scripts(self, tmp_path, script_type):
        """Skills agents get /pdca-<name> in shared script hints."""
        from pdca_cli import _install_shared_infra
    ...
    def test_full_init_claude_resolves_page_templates(self, tmp_path):
        ...
        plan = project / ".pdca" / "templates" / "plan-template.md"
        content = plan.read_text(encoding="utf-8")
        assert "/pdca-plan" in content, "Claude (skills) should use /pdca-plan"
        assert "__PDCA_COMMAND_" not in content
    ...
    def test_full_init_copilot_resolves_page_templates(self, tmp_path):
        ...
        plan = project / ".pdca" / "templates" / "plan-template.md"
        content = plan.read_text(encoding="utf-8")
        assert "/pdca.plan" in content, "Copilot (markdown) should use /pdca.plan"
        assert "__PDCA_COMMAND_" not in content
    ...
    def test_full_init_copilot_skills_resolves_page_templates(self, tmp_path):
        ...
        plan = project / ".pdca" / "templates" / "plan-template.md"
        content = plan.read_text(encoding="utf-8")
        assert "/pdca-plan" in content, "Copilot --skills should use /pdca-plan"
        assert "/pdca.plan" not in content, "dot-notation leaked into Copilot skills page template"
        assert "__PDCA_COMMAND_" not in content
...
class TestIntegrationCatalogDiscoveryCLI:
    ...
    def _patch_catalog(self, monkeypatch, integrations=None):
        ...
        from pdca_cli.integrations.catalog import IntegrationCatalog
    ...
    def test_integration_install_failure_reports_phase_target_and_rollback(
        ...
        from pdca_cli.integrations import INTEGRATION_REGISTRY
        from pdca_cli.integrations.base import IntegrationBase
    ...
    def test_integration_upgrade_failure_reports_phase_and_target(
        ...
        (project / ".pdca" / "integrations").mkdir(parents=True, exist_ok=True)
        (project / ".pdca" / "integration.json").write_text(
            json.dumps(
                {
                    "version": 1,
        ...
        (
            project / ".pdca" / "integrations" / "upgrade-broken.manifest.json"
        ).write_text(
            json.dumps(
                {
                    "integration": "upgrade-broken",
    ...
    def test_integration_switch_cleanup_warning_reports_phase_and_targets(
        ...
        from pdca_cli.extensions import ExtensionManager
        ...
        (project / ".pdca" / "integrations" / "copilot.manifest.json").write_text(
            json.dumps(
                {
                    "integration": "copilot",
                    "version": "0.0.0",
    ...
    def test_search_requires_specify_project(self, tmp_path):
        ...
        assert result.exit_code == 1
        assert "Not a pdca-kit project" in result.output
    ...
    def test_primary_integration_commands_require_specify_project(self, tmp_path):
        ...
        for command in commands:
            ...
            assert "Not a pdca-kit project" in result.output, failure_context

    def test_integration_commands_require_specify_directory(self, tmp_path):
        project = tmp_path / "bad"
        project.mkdir()
        (project / ".pdca").write_text("not a directory")
    ...
    def test_catalog_config_output_uses_posix_paths(self, tmp_path):
        ...
        assert "Config saved to .pdca/preset-catalogs.yml" in preset_add.output

        preset_list = self._invoke(["preset", "catalog", "list"], project)
        assert preset_list.exit_code == 0, preset_list.output
        assert "Config: .pdca/preset-catalogs.yml" in preset_list.output
        ...
        assert "Config saved to .pdca/extension-catalogs.yml" in extension_add.output

        extension_list = self._invoke(["extension", "catalog", "list"], project)
        assert extension_list.exit_code == 0, extension_list.output
        assert "Config: .pdca/extension-catalogs.yml" in extension_list.output
    ...
    def test_search_rejects_non_utf8_integration_json_before_catalog_lookup(
        ...
        (project / ".pdca" / "integration.json").write_bytes(b"\xff\xfe\x00\x00")
    ...
    def test_search_local_config_error_shows_local_config_tip(
        ...
        """`integration search` must point at .pdca/integration-catalogs.yml
        for local-config errors (not the generic 'temporarily unavailable')."""
        project = self._make_project(tmp_path)
        monkeypatch.setenv("HOME", str(tmp_path))
        monkeypatch.setenv("USERPROFILE", str(tmp_path))
        monkeypatch.delenv("PDCA_INTEGRATION_CATALOG_URL", raising=False)
        # Corrupt YAML to drive _load_catalog_config -> IntegrationValidationError.
        cfg = project / ".pdca" / "integration-catalogs.yml"
        ...
        assert ".pdca/integration-catalogs.yml" in normalized_output
        assert "~/.pdca/integration-catalogs.yml" in normalized_output
    ...
    def test_search_invalid_env_catalog_url_shows_env_tip(
        ...
        normalized_output = _normalize_cli_output(result.output)
        assert result.exit_code == 1, result.output
        assert "PDCA_INTEGRATION_CATALOG_URL environment variable" in normalized_output
    ...
    def test_search_whitespace_env_catalog_url_uses_generic_catalog_tip(
        ...
        project = self._make_project(tmp_path)
        monkeypatch.setenv("PDCA_INTEGRATION_CATALOG_URL", "   ")

        from pdca_cli.integrations.catalog import (
            IntegrationCatalog,
        ...
        assert (
            "PDCA_INTEGRATION_CATALOG_URL environment variable"
    ...
    def test_info_unknown_with_invalid_env_catalog_url_shows_env_tip(
        ...
        normalized_output = _normalize_cli_output(result.output)
        assert result.exit_code == 1, result.output
        assert "PDCA_INTEGRATION_CATALOG_URL" in normalized_output
        assert "unset it to use the configured catalog files" in normalized_output
        assert "Try again when online" not in normalized_output
    ...
    def test_catalog_add_then_remove_roundtrip(self, tmp_path, monkeypatch):
        ...
        cfg_path = project / ".pdca" / "integration-catalogs.yml"
    ...
    def test_catalog_list_env_override_supersedes_project_config(
        ...
        normalized_output = _normalize_cli_output(result.output)
        assert result.exit_code == 0, result.output
        assert "PDCA_INTEGRATION_CATALOG_URL is set" in normalized_output
