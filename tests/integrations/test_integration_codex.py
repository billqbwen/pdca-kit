...
class TestCodexAutoPromote:
    ...
    def test_ai_codex_without_ai_skills_auto_promotes(self, tmp_path):
        ...
        assert (target / ".agents" / "skills" / "pdca-plan" / "SKILL.md").exists()
...
class TestCodexHookCommandNote:
    ...
    ``pdca.git.commit`` but Codex skills are named with hyphens
    (``pdca-git-commit``). Without this note, Codex emits
    ``/pdca.git.commit``, which does not resolve.
    """
