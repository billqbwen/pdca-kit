...
adapted for the ``pdca-<name>/SKILL.md`` skills layout.
"""
...
from pdca_cli.integrations.base import SkillsIntegration
...
class SkillsIntegrationTests:
    ...
    def test_setup_writes_to_correct_directory(self, tmp_path):
        ...
        for f in skill_files:
            # Each SKILL.md is in pdca-<name>/ under the skills directory
    ...
    def test_skill_directory_structure(self, tmp_path):
        ...
        actual_commands = set()
        for f in skill_files:
            skill_dir_name = f.parent.name  # e.g. "pdca-plan"
    ...
    def test_command_refs_use_hyphen_separator(self, tmp_path):
        ...
        for f in skill_files:
            content = f.read_text(encoding="utf-8")
            # Skills agents must use /pdca-<name>, not /pdca.<name>
            assert "/pdca." not in content, (
                f"{f.name} contains dot-notation /pdca. reference; "
                f"skills agents must use /pdca-<name>"
            )
    ...
    def test_plan_references_correct_context_file(self, tmp_path):
        ...
        m = IntegrationManifest(self.KEY, tmp_path)
        i.setup(tmp_path, m)
        plan_file = i.skills_dest(tmp_path) / "pdca-plan" / "SKILL.md"
    ...
    def test_pre_existing_skills_not_removed(self, tmp_path):
        """Pre-existing non-pdca skills should be left untouched."""
        i = get_integration(self.KEY)
    ...
    def _expected_files(self, script_variant: str) -> list[str]:
        ...
        files += [
            ...
            f".pdca/integrations/{self.KEY}.manifest.json",
