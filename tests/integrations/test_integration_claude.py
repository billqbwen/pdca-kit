...
from pdca_cli.integrations import INTEGRATION_REGISTRY, get_integration
from pdca_cli.integrations.base import IntegrationBase, SkillsIntegration
from pdca_cli.integrations.claude import ARGUMENT_HINTS
...
class TestClaudeIntegration:
    ...
    def test_setup_creates_skill_files(self, tmp_path):
        ...
        assert "__PDCA_COMMAND_" not in content, "unprocessed __PDCA_COMMAND_*__"
        assert "/pdca." not in content, "skills agent must use /pdca-<name> not /pdca.<name>"

        parts = content.split("---", 2)
        parsed = yaml.safe_load(parts[1])
        assert parsed["name"] == "pdca-plan"
    ...
    def test_ai_flag_auto_promotes_and_enables_skills(self, tmp_path):
        ...
        init_options = json.loads(
            (project / ".pdca" / "init-options.json").read_text(encoding="utf-8")
        )
    ...
    def test_interactive_claude_selection_uses_integration_path(self, tmp_path):
        ...
        try:
            ...
            runner = CliRunner()
            with (
                patch("pdca_cli.commands.init._stdin_is_interactive", return_value=True),
                patch("pdca_cli.commands.init.select_with_arrows", return_value="claude"),
            ):
                result = runner.invoke(
                    app,
                    [
        ...
        assert result.exit_code == 0, result.output
        assert (project / ".pdca" / "integration.json").exists()
        assert (project / ".pdca" / "integrations" / "claude.manifest.json").exists()

        skill_file = project / ".claude" / "skills" / "pdca-plan" / "SKILL.md"
    ...
    def test_claude_hooks_render_skill_invocation(self, tmp_path):
        ...
        assert "Executing: `/pdca-plan`" in message
        assert "EXECUTE_COMMAND: pdca.plan" in message
        assert "EXECUTE_COMMAND_INVOCATION: /pdca-plan" in message
...
class TestClaudeDisableModelInvocation:
    ...
    def test_non_claude_agents_lack_disable_model_invocation(self, tmp_path):
        ...
        fm = CommandRegistrar.build_skill_frontmatter(
            "codex", "pdca-plan", "desc", "templates/commands/plan.md"
        )
...
class TestClaudeHookCommandNote:
    ...
    def test_hook_note_fills_missing_repeated_instructions(self, tmp_path):
        ...
        from pdca_cli.integrations.base import _HOOK_COMMAND_NOTE
...
class TestPDCAManifestRecordsSkippedFiles:
    ...
    against an already-populated ``.pdca/`` tree, every file went down the
    ...
    Reproduction (without the fix) using ``install_shared_infra`` directly:
        ...
        (p / ".pdca/integrations/pdca.manifest.json").unlink()
    ...
    def _read_manifest_files(self, project_path: Path) -> dict:
        manifest_path = (
            project_path / ".pdca" / "integrations" / "pdca.manifest.json"
        )
        assert manifest_path.exists(), (
            f"pdca.manifest.json not written at {manifest_path}"
        )
    ...
    def test_install_shared_infra_records_skipped_files(self, tmp_path):
        """With ``force=False`` and ``.pdca/`` already populated, the
        ...
        # Simulate a lost manifest while ``.pdca/`` is still on disk
        # (e.g. the manifest was deleted, corrupted, or the layout was
        # extracted out-of-band).
        manifest_path = (
            tmp_path / ".pdca" / "integrations" / "pdca.manifest.json"
        )
        ...
        assert second_files, (
            "pdca.manifest.json files dict is empty after install with "
    ...
    def test_install_shared_infra_handles_directory_at_script_destination(
        ...
        # Pre-create the .pdca/scripts/bash tree, then plant a directory
        # where a script file is expected so the skip branch hits a
        # non-regular-file path.
        bash_dir = tmp_path / ".pdca" / "scripts" / "bash"
        ...
        assert ".pdca/scripts/bash/common.sh" not in files, (
    ...
    def test_install_shared_infra_handles_directory_at_template_destination(
        ...
        templates_dir = tmp_path / ".pdca" / "templates"
        ...
        files = self._read_manifest_files(tmp_path)
        template_rel = f".pdca/templates/{real_template}"
