...
class TestCopilotIntegration:
    ...
    def test_command_filename_agent_md(self):
        copilot = get_integration("copilot")
        assert copilot.command_filename("plan") == "pdca.plan.agent.md"
    ...
    def test_templates_are_processed(self, tmp_path):
        ...
        copilot.setup(tmp_path, m)
        agents_dir = tmp_path / ".github" / "agents"
        for agent_file in agents_dir.glob("pdca.*.agent.md"):
    ...
    def test_plan_references_correct_context_file(self, tmp_path):
        ...
        m = IntegrationManifest("copilot", tmp_path)
        copilot.setup(tmp_path, m)
        plan_file = tmp_path / ".github" / "agents" / "pdca.plan.agent.md"
    ...
    def test_complete_file_inventory_sh(self, tmp_path):
        ...
        expected = sorted([
            ...
            ".pdca/integrations/pdca.manifest.json",
            ".pdca/scripts/bash/check-prerequisites.sh",
            ".pdca/scripts/bash/common.sh",
            ".pdca/scripts/bash/create-new-feature.sh",
            ".pdca/scripts/bash/setup-plan.sh",
            ".pdca/scripts/bash/setup-tasks.sh",
            ".pdca/templates/checklist-template.md",
            ".pdca/templates/constitution-template.md",
            ".pdca/templates/plan-template.md",
            ".pdca/templates/spec-template.md",
            ".pdca/templates/tasks-template.md",
            ".pdca/memory/constitution.md",
    ...
    def test_complete_file_inventory_ps(self, tmp_path):
        ...
        expected = sorted([
            ...
            ".pdca/integrations/copilot.manifest.json",
            ".pdca/integrations/pdca.manifest.json",
            ".pdca/scripts/powershell/check-prerequisites.ps1",
            ".pdca/scripts/powershell/common.ps1",
            ".pdca/scripts/powershell/create-new-feature.ps1",
            ".pdca/scripts/powershell/setup-plan.ps1",
            ".pdca/scripts/powershell/setup-tasks.ps1",
            ".pdca/templates/checklist-template.md",
            ".pdca/templates/constitution-template.md",
            ".pdca/templates/plan-template.md",
            ".pdca/templates/spec-template.md",
...
class TestCopilotSkillsMode:
    ...
    def test_skills_creates_skill_files(self, tmp_path):
        ...
        for f in skill_files:
            assert f.exists()
            assert f.parent.name.startswith("pdca-")
    ...
    def test_skills_directory_structure(self, tmp_path):
        """Each command produces pdca-<name>/SKILL.md."""
        copilot = self._make_copilot()
        created, _ = self._setup_skills(copilot, tmp_path)
        skill_files = [f for f in created if f.name == "SKILL.md"]
        ...
        for f in skill_files:
            ...
            assert skill_dir_name.startswith("pdca-")
            actual_commands.add(skill_dir_name.removeprefix("pdca-"))
        assert actual_commands == expected_commands
    ...
    def test_skills_templates_are_processed(self, tmp_path):
        ...
        for f in skill_files:
            ...
            assert "__PDCA_COMMAND_" not in content, f"{f.name} has unprocessed __PDCA_COMMAND_*__"

    def test_skills_command_refs_use_hyphen(self, tmp_path):
        """Copilot skills mode must use /pdca-<name> not /pdca.<name>."""
        copilot = self._make_copilot()
        created, _ = self._setup_skills(copilot, tmp_path)
        skill_files = [f for f in created if f.name == "SKILL.md"]
        assert len(skill_files) > 0
        for f in skill_files:
            content = f.read_text(encoding="utf-8")
            assert "/pdca." not in content, (
                f"{f.name} contains dot-notation /pdca. reference; "
                f"skills mode must use /pdca-<name>"
            )
    ...
    def test_plan_references_correct_context_file(self, tmp_path):
        ...
        copilot = self._make_copilot()
        self._setup_skills(copilot, tmp_path)
        plan_file = tmp_path / ".github" / "skills" / "pdca-plan" / "SKILL.md"
        assert plan_file.exists()
        content = plan_file.read_text(encoding="utf-8")
        assert copilot.context_file in content
        assert "__CONTEXT_FILE__" not in content
    ...
    def test_build_command_invocation_skills_mode(self):
        ...
        assert copilot.build_command_invocation("pdca.plan") == "/pdca-plan"
        assert copilot.build_command_invocation("plan") == "/pdca-plan"
        assert copilot.build_command_invocation("plan", "my args") == "/pdca-plan my args"

    def test_build_command_invocation_skills_extension_command(self):
        copilot = self._make_copilot()
        copilot._skills_mode = True
        assert copilot.build_command_invocation("pdca.git.commit") == "/pdca-git-commit"
        assert copilot.build_command_invocation("git.commit") == "/pdca-git-commit"
    ...
    def test_init_with_integration_options_skills(self, tmp_path):
        ...
        plan_skill = skills_dir / "pdca-plan" / "SKILL.md"
        assert plan_skill.exists(), "pdca-plan/SKILL.md not found"
    ...
    def test_dispatch_ignores_unrelated_skills_directory(self, tmp_path):
        ...
        copilot = self._make_copilot()
        # Create a .github/skills/ with non-pdca content (e.g. GitHub Skills training)
        ...
        with mock.patch("subprocess.run") as mock_run:
            ...
            assert "pdca.plan" in call_args

    def test_dispatch_detects_pdca_skills_layout(self, tmp_path):
        """dispatch_command() detects pdca-*/SKILL.md as skills mode."""
        copilot = self._make_copilot()
        skill_dir = tmp_path / ".github" / "skills" / "pdca-plan"
        ...
        with mock.patch("subprocess.run") as mock_run:
            ...
            assert "/pdca-plan" in prompt, (
                f"Skills mode prompt should invoke /pdca-plan, got: {prompt}"
            )
    ...
    def test_init_skills_next_steps_show_skill_syntax(self, tmp_path):
        """specify init --integration copilot --integration-options='--skills' shows /pdca-plan not /pdca.plan."""
        from typer.testing import CliRunner
        from pdca_cli import app
        ...
        try:
            ...
            os.chdir(old_cwd)
        assert result.exit_code == 0, f"init failed: {result.output}"
        # Skills mode should show /pdca-plan (hyphenated)
        assert "/pdca-plan" in result.output, (
            f"Expected /pdca-plan in next steps but got:\n{result.output}"
        )
        # Must NOT show the dotted /pdca.plan form
        assert "/pdca.plan" not in result.output, (
            f"Should not show /pdca.plan in skills mode:\n{result.output}"
        )
