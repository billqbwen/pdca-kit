...
class TestCatalogFetch:
    ...
    def _patch_urlopen(self, monkeypatch, catalog_data):
        ...
        import pdca_cli.authentication.http as _auth_http
...
VALID_DESCRIPTOR = {
    ...
    "provides": {
        "commands": [
            {"name": "pdca.pdca", "file": "templates/pdca.define.md"},
        ],
        "scripts": [],
    },
}
...
class TestIntegrationDescriptor:
    ...
    def test_valid_descriptor(self, tmp_path):
        ...
        assert desc.requires_pdca_version == ">=0.6.0"
    ...
    def test_missing_pdca_version(self, tmp_path):
        data = {**VALID_DESCRIPTOR, "requires": {}}
        p = self._write(tmp_path, data)
        with pytest.raises(IntegrationDescriptorError, match="requires.pdca_version"):
            IntegrationDescriptor(p)
...
class TestIntegrationListCatalog:
    ...
    def _init_project(self, tmp_path):
        """Create a minimal pdca-kit project."""
...
class TestIntegrationUpgrade:
    ...
    def test_upgrade_requires_pdca_project(self, tmp_path):
        from typer.testing import CliRunner
        from pdca_cli import app
        runner = CliRunner()
        old = os.getcwd()
    ...
    def test_upgrade_blocks_on_modified_files(self, tmp_path):
        ...
        manifest_path = project / ".pdca" / "integrations" / "copilot.manifest.json"
...
class TestCatalogSourceManagement:
    ...
    def test_add_catalog_creates_config_file(self, tmp_path, monkeypatch):
        ...
        cfg_path = tmp_path / ".pdca" / "integration-catalogs.yml"
    ...
    def test_add_catalog_auto_derives_name_and_priority(self, tmp_path, monkeypatch):
        ...
        data = yaml.safe_load(
            (tmp_path / ".pdca" / "integration-catalogs.yml").read_text(encoding="utf-8")
        )
    ...
    def test_add_catalog_rejects_invalid_url(self, tmp_path, monkeypatch):
        ...
        assert not (tmp_path / ".pdca" / "integration-catalogs.yml").exists()
