...
from pdca_cli.integrations.forge import format_forge_command_name
...
class TestForgeCommandNameFormatter:
    ...
    def test_name_with_pdca_prefix(self):
        ...
        assert format_forge_command_name("pdca.plan") == "pdca-plan"
        assert format_forge_command_name("pdca.tasks") == "pdca-tasks"

    def test_extension_command_name(self):
        """Test formatting extension command names with dots."""
        assert format_forge_command_name("pdca.my-extension.example") == "pdca-my-extension-example"
        assert format_forge_command_name("my-extension.example") == "pdca-my-extension-example"

    def test_complex_nested_name(self):
        """Test formatting deeply nested command names."""
        assert format_forge_command_name("pdca.jira.sync-status") == "pdca-jira-sync-status"
        assert format_forge_command_name("pdca.foo.bar.baz") == "pdca-foo-bar-baz"

    def test_name_with_hyphens_preserved(self):
        """Test that existing hyphens are preserved."""
        assert format_forge_command_name("my-extension") == "pdca-my-extension"
        assert format_forge_command_name("pdca.my-ext.test-cmd") == "pdca-my-ext-test-cmd"

    def test_alias_formatting(self):
        """Test formatting alias names."""
        assert format_forge_command_name("pdca.my-extension.example-short") == "pdca-my-extension-example-short"
...
class TestForgeIntegration:
    ...
    def test_command_filename_md(self):
        forge = get_integration("forge")
        assert forge.command_filename("plan") == "pdca.plan.md"
    ...
    def test_directory_structure(self, tmp_path):
        ...
        command_files = sorted(commands_dir.glob("pdca.*.md"))
        assert len(command_files) == len(expected_commands)
        actual_commands = {f.name.removeprefix("pdca.").removesuffix(".md") for f in command_files}
    ...
    def test_templates_are_processed(self, tmp_path):
        ...
        forge.setup(tmp_path, m)
        commands_dir = tmp_path / ".forge" / "commands"
        for cmd_file in commands_dir.glob("pdca.*.md"):
            ...
            assert "__PDCA_COMMAND_" not in content, f"{cmd_file.name} has unprocessed __PDCA_COMMAND_*__"
            ...
            assert not re.search(r"/pdca\.[a-z]", content), (
                f"{cmd_file.name} contains dot-notation command reference (/pdca.<cmd>); "
                "Forge requires hyphen notation (/pdca-<cmd>) for ZSH compatibility"
            )
    ...
    def test_plan_references_correct_context_file(self, tmp_path):
        ...
        m = IntegrationManifest("forge", tmp_path)
        forge.setup(tmp_path, m)
        plan_file = tmp_path / ".forge" / "commands" / "pdca.plan.md"
    ...
    def test_uses_parameters_placeholder(self, tmp_path):
        ...
        checklist = commands_dir / "pdca.checklist.md"
        if checklist.exists():
    ...
    def test_command_refs_use_hyphen_notation(self, tmp_path):
        """Verify all generated Forge command files use /pdca-foo, not /pdca.foo."""
        import re
        from pdca_cli.integrations.forge import ForgeIntegration
        forge = ForgeIntegration()
        m = IntegrationManifest("forge", tmp_path)
        ...
        for cmd_file in commands_dir.glob("pdca.*.md"):
            content = cmd_file.read_text(encoding="utf-8")
            if re.search(r"/pdca-[a-z]", content):
        ...
        assert len(files_with_refs) > 0, (
            "Expected at least one generated Forge command to contain /pdca-<cmd> reference, "
            "but none were found. Check that __PDCA_COMMAND_*__ tokens are being resolved."
        )

    def test_name_field_uses_hyphenated_format(self, tmp_path):
        """Verify that injected name fields use hyphenated format (pdca-plan, not pdca.plan)."""
        from pdca_cli.integrations.forge import ForgeIntegration
        from pdca_cli.agents import CommandRegistrar
        forge = ForgeIntegration()
        m = IntegrationManifest("forge", tmp_path)
        ...
        for cmd_file in commands_dir.glob("pdca.*.md"):
            ...
            assert name_value.startswith("pdca-"), (
                f"{cmd_file.name} name field should start with 'pdca-': {name_value}"
            )
...
class TestForgeCommandRegistrar:
    ...
    def test_registrar_formats_extension_command_names_for_forge(self, tmp_path):
        ...
        commands = [
            {
                "name": "pdca.my-extension.example",
        ...
        assert "pdca.my-extension.example" in registered
        
        # Check the generated file has hyphenated name in frontmatter
        forge_cmd = tmp_path / ".forge" / "commands" / "pdca-my-extension-example.md"
        ...
        assert frontmatter["name"] == "pdca-my-extension-example"
    ...
    def test_registrar_formats_alias_names_for_forge(self, tmp_path):
        ...
        alias_file = tmp_path / ".forge" / "commands" / "pdca-my-extension-ex.md"
        ...
        assert frontmatter["name"] == "pdca-my-extension-ex"
    ...
    def test_registrar_does_not_affect_other_agents(self, tmp_path):
        ...
        windsurf_cmd = tmp_path / ".windsurf" / "workflows" / "pdca.my-extension.example.md"
    ...
    def test_git_extension_command_uses_hyphen_notation(self, tmp_path):
        ...
        cmd_source = ext_dir / "commands" / "pdca.git.feature.md"
        assert cmd_source.exists(), (
            f"Git extension command source not found at {cmd_source}. "
            "Ensure extensions/git/commands/pdca.git.feature.md exists."
        )

        registrar = CommandRegistrar()
        commands = [
            {
                "name": "pdca.git.feature",
                "file": "commands/pdca.git.feature.md",
            }
        ]
        ...
        assert "pdca.git.feature" in registered

        forge_cmd = tmp_path / ".forge" / "commands" / "pdca-git-feature.md"
