...
class TestIntegrationBase:
    ...
    def test_setup_copies_templates(self, tmp_path, monkeypatch):
        ...
        assert (project / ".stub" / "commands" / "pdca.plan.md").exists()
        assert (project / ".stub" / "commands" / "pdca.define.md").exists()
...
class TestBasePrimitives:
    ...
    def test_command_filename_default(self):
        i = StubIntegration()
        assert i.command_filename("plan") == "pdca.plan.md"
    ...
    def test_copy_command_to_directory(self, tmp_path):
        ...
        result = IntegrationBase.copy_command_to_directory(src, dest_dir, "pdca.plan.md")
        assert result == dest_dir / "pdca.plan.md"
...
class TestBuildCommandInvocation:
    ...
    def test_base_core_command_dotted(self):
        i = StubIntegration()
        assert i.build_command_invocation("pdca.plan") == "/pdca.plan"

    def test_base_core_command_bare(self):
        i = StubIntegration()
        assert i.build_command_invocation("plan") == "/pdca.plan"

    def test_base_core_command_with_args(self):
        i = StubIntegration()
        assert i.build_command_invocation("plan", "my feature") == "/pdca.plan my feature"

    def test_base_extension_command(self):
        i = StubIntegration()
        assert i.build_command_invocation("pdca.git.commit") == "/pdca.git.commit"

    def test_base_extension_command_bare(self):
        i = StubIntegration()
        assert i.build_command_invocation("git.commit") == "/pdca.git.commit"
    ...
    def test_skills_extension_command_with_args(self):
        from pdca_cli.integrations import get_integration
        i = get_integration("codex")
        assert i.build_command_invocation("pdca.git.commit", "fix typo") == "/pdca-git-commit fix typo"


class TestResolveCommandRefs:
    """Tests for __PDCA_COMMAND_<NAME>__ placeholder resolution."""

    def test_dot_separator_core_command(self):
        text = "Run `__PDCA_COMMAND_PLAN__` to plan."
    ...
    def test_extension_command_dot(self):
        ...
        result = IntegrationBase.resolve_command_refs(text, ".")
        assert result == "Run /pdca.git.commit to commit."
    ...
    def test_default_separator_is_dot(self):
        text = "__PDCA_COMMAND_PLAN__"
        assert IntegrationBase.resolve_command_refs(text) == "/pdca.plan"
    ...
    def test_process_template_resolves_placeholders(self):
        content = "---\ndescription: test\n---\nRun __PDCA_COMMAND_PLAN__ now."
        result = IntegrationBase.process_template(
            content, "test-agent", "sh", invoke_separator="."
        )
        assert "/pdca.plan" in result
        assert "__PDCA_COMMAND_" not in result
    ...
    def test_unclosed_placeholder_unchanged(self):
        text = "Run __PDCA_COMMAND_PLAN to plan."
        assert IntegrationBase.resolve_command_refs(text, ".") == text

    def test_empty_name_not_matched(self):
        text = "Run __PDCA_COMMAND___ to plan."
        assert IntegrationBase.resolve_command_refs(text, ".") == text
    ...
    def test_placeholder_adjacent_to_text(self):
        text = "foo__PDCA_COMMAND_PLAN__bar"
        result = IntegrationBase.resolve_command_refs(text, ".")
        assert result == "foo/pdca.planbar"

    def test_placeholder_with_digits(self):
        text = "__PDCA_COMMAND_V2_PLAN__"
        result = IntegrationBase.resolve_command_refs(text, ".")
        assert result == "/pdca.v2.plan"
