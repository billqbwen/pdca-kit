...
from pdca_cli.integrations.kimi import _migrate_legacy_kimi_dotted_skills
...
class TestKimiLegacyMigration:
    ...
    def test_migrate_dotted_to_hyphenated(self, tmp_path):
        ...
        assert (skills_dir / "pdca-plan" / "SKILL.md").exists()
...
class TestKimiNextSteps:
    ...
    def test_next_steps_show_skill_invocation(self, tmp_path):
        """Kimi next-steps guidance should display /skill:pdca-* usage."""
        import os
        ...
        assert "/skill:pdca-constitution" in result.output
        assert "/pdca.constitution" not in result.output
