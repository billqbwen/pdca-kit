...
def project_dir(temp_dir):
    """Create a mock pdca-kit project with .pdca/ directory."""
    specify_dir = temp_dir / ".pdca"
    specify_dir.mkdir()
...
def sample_workflow_file(project_dir, sample_workflow_yaml):
    ...
    wf_dir = project_dir / ".pdca" / "workflows" / "test-workflow"
...
class TestStepRegistry:
    ...
    def test_registry_populated(self):
        from pdca_cli.workflows import STEP_REGISTRY
    ...
    def test_get_step_type(self):
        from pdca_cli.workflows import get_step_type
    ...
    def test_register_step_duplicate_raises(self):
        from pdca_cli.workflows import _register_step
        from pdca_cli.workflows.steps.command import CommandStep
    ...
    def test_register_step_empty_key_raises(self):
        from pdca_cli.workflows import _register_step
        from pdca_cli.workflows.base import StepBase, StepResult
...
class TestBaseClasses:
    ...
    def test_step_result_defaults(self):
        from pdca_cli.workflows.base import StepResult, StepStatus
    ...
    def test_step_status_values(self):
        from pdca_cli.workflows.base import StepStatus
    ...
    def test_run_status_values(self):
        from pdca_cli.workflows.base import RunStatus
...
class TestExpressions:
    ...
    def test_simple_variable(self):
        from pdca_cli.workflows.expressions import evaluate_expression
        from pdca_cli.workflows.base import StepContext
    ...
    def test_condition_evaluation(self):
        from pdca_cli.workflows.expressions import evaluate_condition
        from pdca_cli.workflows.base import StepContext
...
class TestBuildExecArgs:
    ...
    def test_claude_exec_args(self):
        from pdca_cli.integrations.claude import ClaudeIntegration
    ...
    def test_gemini_exec_args(self):
        from pdca_cli.integrations.gemini import GeminiIntegration
    ...
    def test_codex_exec_args(self):
        from pdca_cli.integrations.codex import CodexIntegration
    ...
    def test_copilot_exec_args(self, monkeypatch):
        monkeypatch.delenv("PDCA_COPILOT_ALLOW_ALL_TOOLS", raising=False)
        monkeypatch.delenv("PDCA_ALLOW_ALL_TOOLS", raising=False)
    ...
    def test_copilot_new_env_var_disables_yolo(self, monkeypatch):
        monkeypatch.setenv("PDCA_COPILOT_ALLOW_ALL_TOOLS", "0")
        monkeypatch.delenv("PDCA_ALLOW_ALL_TOOLS", raising=False)
        from pdca_cli.integrations.copilot import CopilotIntegration
        impl = CopilotIntegration()
        args = impl.build_exec_args("do stuff")
    ...
    def test_copilot_deprecated_env_var_still_honoured(self, monkeypatch):
        monkeypatch.delenv("PDCA_COPILOT_ALLOW_ALL_TOOLS", raising=False)
        monkeypatch.setenv("PDCA_ALLOW_ALL_TOOLS", "0")
        import warnings
        from pdca_cli.integrations.copilot import CopilotIntegration
        impl = CopilotIntegration()
        ...
        assert "--yolo" not in args
        assert any(
            "PDCA_ALLOW_ALL_TOOLS is deprecated" in str(x.message)
            and issubclass(x.category, UserWarning)
            for x in w
        )

    def test_copilot_new_env_var_takes_precedence(self, monkeypatch):
        monkeypatch.setenv("PDCA_COPILOT_ALLOW_ALL_TOOLS", "1")
        monkeypatch.setenv("PDCA_ALLOW_ALL_TOOLS", "0")
        from pdca_cli.integrations.copilot import CopilotIntegration
        impl = CopilotIntegration()
        args = impl.build_exec_args("do stuff")
    ...
    def test_ide_only_returns_none(self):
        from pdca_cli.integrations.windsurf import WindsurfIntegration
...
class TestCommandStep:
    ...
    def test_execute_basic(self):
        ...
        from pdca_cli.workflows.steps.command import CommandStep
        from pdca_cli.workflows.base import StepContext, StepStatus
    ...
    def test_dispatch_with_mock_cli(self, tmp_path, monkeypatch):
        ...
        with patch("pdca_cli.workflows.steps.command.shutil.which", return_value="/usr/local/bin/claude"), \
...
class TestPromptStep:
    ...
    def test_execute_basic(self):
        from unittest.mock import patch
        from pdca_cli.workflows.steps.prompt import PromptStep
        from pdca_cli.workflows.base import StepContext, StepStatus
        ...
        with patch("pdca_cli.workflows.steps.prompt.shutil.which", return_value=None):
            result = step.execute(config, ctx)
        assert result.status == StepStatus.FAILED
    ...
    def test_dispatch_with_mock_cli(self, tmp_path):
        ...
        with patch("pdca_cli.workflows.steps.prompt.shutil.which", return_value="/usr/local/bin/claude"), \
             patch("subprocess.run", return_value=mock_result):
            result = step.execute(config, ctx)
...
class TestShellStep:
    ...
    def test_execute_echo(self):
        from pdca_cli.workflows.steps.shell import ShellStep
        from pdca_cli.workflows.base import StepContext, StepStatus
...
class TestGateStep:
    ...
    def test_execute_returns_paused(self):
        from pdca_cli.workflows.steps.gate import GateStep
        from pdca_cli.workflows.base import StepContext, StepStatus
...
class TestIfThenStep:
    ...
    def test_execute_then_branch(self):
        from pdca_cli.workflows.steps.if_then import IfThenStep
        from pdca_cli.workflows.base import StepContext
        ...
        config = {
            ...
            "then": [{"id": "a", "command": "pdca.tasks"}],
            "else": [{"id": "b", "command": "pdca.plan"}],
        }
        result = step.execute(config, ctx)
...
class TestSwitchStep:
    ...
    def test_execute_matches_case(self):
        from pdca_cli.workflows.steps.switch import SwitchStep
        from pdca_cli.workflows.base import StepContext
        ...
        config = {
            ...
            "cases": {
                "approve": [{"id": "plan", "command": "pdca.plan"}],
...
class TestWhileStep:
    ...
    def test_execute_condition_true(self):
        from pdca_cli.workflows.steps.while_loop import WhileStep
        from pdca_cli.workflows.base import StepContext
        ...
        config = {
            ...
            "max_iterations": 5,
            "steps": [{"id": "fix", "command": "pdca.implement"}],
        }
        result = step.execute(config, ctx)
        assert result.output["condition_result"] is True
        assert len(result.next_steps) == 1
...
class TestDoWhileStep:
    ...
    def test_execute_always_runs_once(self):
        from pdca_cli.workflows.steps.do_while import DoWhileStep
        from pdca_cli.workflows.base import StepContext
    ...
    def test_execute_with_true_condition(self):
        ...
        config = {
            ...
            "condition": "{{ true }}",
            "max_iterations": 5,
            "steps": [{"id": "work", "command": "pdca.plan"}],
        }
        result = step.execute(config, ctx)
...
class TestFanOutStep:
    ...
    def test_execute_with_items(self):
        from pdca_cli.workflows.steps.fan_out import FanOutStep
        from pdca_cli.workflows.base import StepContext
        ...
        config = {
            ...
            "max_concurrency": 3,
            "step": {"id": "impl", "command": "pdca.implement"},
        }
        result = step.execute(config, ctx)
...
class TestFanInStep:
    ...
    def test_execute_collects_results(self):
        from pdca_cli.workflows.steps.fan_in import FanInStep
        from pdca_cli.workflows.base import StepContext
...
class TestWorkflowDefinition:
    ...
    def test_from_yaml(self, sample_workflow_file):
        from pdca_cli.workflows.engine import WorkflowDefinition
...
class TestWorkflowValidation:
    ...
    def test_valid_workflow(self, sample_workflow_yaml):
        from pdca_cli.workflows.engine import WorkflowDefinition, validate_workflow
...
class TestWorkflowEngine:
    ...
    def test_load_from_file(self, sample_workflow_file, project_dir):
        from pdca_cli.workflows.engine import WorkflowEngine
    ...
    def test_execute_simple_workflow(self, project_dir):
        from unittest.mock import patch
        from pdca_cli.workflows.engine import WorkflowEngine, WorkflowDefinition
        from pdca_cli.workflows.base import RunStatus
...
"""
    ...
    def test_integration_auto_default_uses_project_integration(self, project_dir):
        """`integration: auto` should resolve to .pdca/integration.json's integration."""
        from pdca_cli.workflows.engine import WorkflowEngine, WorkflowDefinition

        specify_dir = project_dir / ".pdca"
...
""")
    ...
    def test_integration_auto_default_falls_back_when_no_integration_json(self, project_dir):
        ...
        ``.pdca/integration.json`` is absent; any later validation or
...
""")
    ...
    def test_integration_auto_rejects_future_state_schema(self, project_dir):
        ...
        from pdca_cli.workflows.engine import WorkflowEngine, WorkflowDefinition
        from pdca_cli.integration_state import INTEGRATION_STATE_SCHEMA
...
""")
    ...
    def test_while_loop_condition_reads_latest_iteration(self, project_dir):
        ...
        See https://github.com/github/pdca-kit/issues/2592
        """
        from pdca_cli.workflows.engine import WorkflowEngine, WorkflowDefinition
        from pdca_cli.workflows.base import RunStatus
...
class TestContextRunId:
    ...
    def test_shell_run_resolves_run_id(self, project_dir):
        ...
        """
        from pdca_cli.workflows.engine import WorkflowDefinition, WorkflowEngine
...
""")
        engine = WorkflowEngine(project_dir)
        with patch(
            "pdca_cli.workflows.steps.command.shutil.which",
            return_value=None,
        ):
...
""")
    ...
    def test_run_id_uses_pdca_workflow_run_id_env_override(self, project_dir, monkeypatch):
        """When no run_id argument is provided, PDCA_WORKFLOW_RUN_ID overrides the auto-generated run ID."""
        from pdca_cli.workflows.engine import WorkflowDefinition, WorkflowEngine

        monkeypatch.setenv("PDCA_WORKFLOW_RUN_ID", "env-run-123")
        definition = WorkflowDefinition.from_string("""
schema_version: "1.0"
workflow:
  id: "env-run-id"
...
""")
    ...
    def test_run_id_arg_takes_precedence_over_env_override(self, project_dir, monkeypatch):
        """Explicit run_id keeps existing precedence over PDCA_WORKFLOW_RUN_ID."""
        from pdca_cli.workflows.engine import WorkflowDefinition, WorkflowEngine
...
""")
    ...
    def test_gate_abort_still_halts_with_continue_on_error(
        ...
        from pdca_cli.workflows.base import RunStatus
        from pdca_cli.workflows.steps.gate import GateStep
        from pdca_cli.workflows.steps import gate as gate_module
...
""")
    ...
    def test_validation_rejects_non_bool_continue_on_error(self):
        ...
        """
        from pdca_cli.workflows.engine import (
            WorkflowDefinition,
            validate_workflow,
        )
...
class TestRunState:
    ...
    def test_save_and_load(self, project_dir):
        from pdca_cli.workflows.engine import RunState
        from pdca_cli.workflows.base import RunStatus
...
class TestWorkflowRegistry:
    ...
    def test_add_and_get(self, project_dir):
        from pdca_cli.workflows.catalog import WorkflowRegistry
...
class TestWorkflowCatalog:
    ...
    def test_default_catalogs(self, project_dir):
        from pdca_cli.workflows.catalog import WorkflowCatalog
    ...
    def test_env_var_override(self, project_dir, monkeypatch):
        ...
        monkeypatch.setenv("PDCA_WORKFLOW_CATALOG_URL", "https://example.com/catalog.json")
        catalog = WorkflowCatalog(project_dir)
        entries = catalog.get_active_catalogs()
    ...
    def test_project_level_config(self, project_dir):
        ...
        config_path = project_dir / ".pdca" / "workflow-catalogs.yml"
    ...
    def test_validate_url_http_rejected(self, project_dir):
        from pdca_cli.workflows.catalog import WorkflowCatalog, WorkflowValidationError
...
steps:
  - id: specify
    type: shell
    run: "echo pdca.pdca {{ inputs.feature }}"
