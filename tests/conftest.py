"""Shared test helpers for the PDCA Kit test suite."""
...
def _has_working_bash() -> bool:
    ...
    Set PDCA_TEST_BASH=1 to force-enable bash tests regardless.
    """
    if os.environ.get("PDCA_TEST_BASH") == "1":
        return True
...
# ---------------------------------------------------------------------------
# Auth config isolation — prevents tests from reading ~/.pdca/auth.json
# ---------------------------------------------------------------------------


@pytest.fixture(autouse=True)
def _isolate_auth_config(monkeypatch):
    """Ensure no test reads the real ~/.pdca/auth.json."""
    from pdca_cli.authentication import http as _auth_http
