...
If `$ARGUMENTS` is empty, you must tell the user to provide an extension name, for example: `/pdca.selftest.extension linear`.

## Steps

### Step 1: Catalog Discovery Validation

Check if the extension exists in the PDCA Kit catalog.
...
```bash
pdca extension add "$ARGUMENTS" --from "<download_url>"
```
...
```bash
cat .pdca/extensions/.registry/$ARGUMENTS.json
```
...
test_selftest_discovery.py::test_catalog_search [PASS/FAIL]
  Details: [Provide execution result of pdca extension search]

test_selftest_installation.py::test_extension_add [PASS/FAIL]
  Details: [Provide execution result of pdca extension add]
