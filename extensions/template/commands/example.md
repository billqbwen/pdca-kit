...
This is an example command that demonstrates how to create commands for PDCA Kit extensions.
...
``bash
config_file=".pdca/extensions/my-extension/my-extension-config.yml"

if [ ! -f "$config_file" ]; then
  echo "❌ Error: Configuration not found at $config_file"
  echo "Run 'pdca extension add my-extension' to install and configure"
  exit 1
fi
...
setting_value="${PDCA_MY_EXTENSION_KEY:-$setting_value}"
...
``bash
output_file=".pdca/my-extension-output.json"
...
- `PDCA_MY_EXTENSION_KEY` - Overrides `settings.key`
- `PDCA_MY_EXTENSION_ANOTHER_KEY` - Overrides `settings.another_key`

Example:
``bash
export PDCA_MY_EXTENSION_KEY="override-value"
``
...
``bash
pdca extension add my-extension
cp .pdca/extensions/my-extension/config-template.yml \
   .pdca/extensions/my-extension/my-extension-config.yml
``
...
export PDCA_MY_EXTENSION_KEY="custom-value"
> /pdca.my-extension.example
``
...
*For more information, see the extension README or run `pdca extension info my-extension`*
