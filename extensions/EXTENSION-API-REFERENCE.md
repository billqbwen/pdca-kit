...
Technical reference for PDCA Kit extension system APIs and manifest schema.
...
requires:
  pdca_version: string  # Required, version specifier (>=X.Y.Z)
...
provides:
  commands:              # Required, at least one command
    - name: string       # Required, pattern: ^pdca\.[a-z0-9-]+\.[a-z0-9-]+$
...
#### `requires.pdca_version`

- **Type**: string
- **Format**: Version specifier
- **Description**: Required pdca-kit version range
- **Examples**:
...
- **Format**:  `pdca.{extension-id}.{command-name}`
- **Examples**: `pdca.jira.specstoissues`, `pdca.linear.sync`
- **Invalid**: `jira.specstoissues`, `pdca.command`, `pdca.jira.CreateIssues`
...
- **Events**: Defined by core pdca-kit commands
...
manifest.requires_pdca_version  # str: Required pdca-kit version
...
CompatibilityError    # Incompatible with current pdca-kit version
```
...
```json
{
  ...
  "extensions": {
    "jira": {
      ...
      "enabled": true,
      "registered_commands": ["pdca.jira.specstoissues", ...],
...
```python
from pdca_cli.extensions import CatalogEntry
...
```python
from pdca_cli.extensions import ExtensionCatalog
...
**Catalog config file** (`.pdca/extension-catalogs.yml`):
...
```python
from pdca_cli.extensions import CommandRegistrar
...
- Extension context automatically injected:
  ...
  <!-- Config: .pdca/extensions/{extension-id}/ -->
  ```
...
**File**: `.pdca/extensions/{extension-id}/{extension-id}-config.yml`
...
4. **Environment Variables** (`PDCA_{EXTENSION}_*`)

### Environment Variable Pattern

Format: `PDCA_{EXTENSION}_{KEY}`
...
**Usage**: `pdca extension list [OPTIONS]`
...
**Usage**: `pdca extension catalog list`
...
**Usage**: `pdca extension catalog add URL [OPTIONS]`
...
Adds a catalog entry to `.pdca/extension-catalogs.yml`.

### extension catalog remove

**Usage**: `pdca extension catalog remove NAME`

**Arguments**:

- `NAME` - Catalog name to remove

Removes a catalog entry from `.pdca/extension-catalogs.yml`.

### extension add

**Usage**: `pdca extension add EXTENSION [OPTIONS]`
...
**Usage**: `pdca extension remove EXTENSION [OPTIONS]`
...
**Usage**: `pdca extension search [QUERY] [OPTIONS]`
...
**Usage**: `pdca extension info EXTENSION`
...
**Usage**: `pdca extension update [EXTENSION]`
...
**Usage**: `pdca extension enable EXTENSION`
...
**Usage**: `pdca extension disable EXTENSION`
...
```python
from pdca_cli.extensions import ValidationError
...
Raised when extension is incompatible with current pdca-kit version.

```python
from pdca_cli.extensions import CompatibilityError
...
```python
from pdca_cli.extensions import ExtensionError
...
```python
from pdca_cli.extensions import version_satisfies
...
└── scripts/                    # (existing pdca-kit)

.claude/
└── commands/
    └── pdca.{ext}.{cmd}.md  # Registered commands
```
