...
Introduce an extension system to PDCA Kit that allows modular integration with external tools (Jira, Linear, Azure DevOps, etc.) without bloating the core framework. Extensions are self-contained packages installed into `.pdca/extensions/` with declarative manifests, versioned independently, and discoverable through a central catalog.
...
1. **Monolithic Growth**: Adding Jira integration to core pdca-kit creates:
...
1. **Modularity**: Core pdca-kit remains lean, extensions are opt-in
...
- Standard directory structure (`.pdca/extensions/{name}/`)
- Declarative manifest (`extension.yml`)
- Predictable command naming (`pdca.{extension}.{command}`)
...
- Simple installation: `pdca extension add jira`
...
extension:
  ...
  description: "Create Jira Epics, Stories, and Issues from pdca-kit artifacts"
  author: "Stats Perform"       # Author/organization
  repository: "https://github.com/statsperform/pdca-kit-jira"
  license: "MIT"                # SPDX license identifier
  homepage: "https://github.com/statsperform/pdca-kit-jira/blob/main/README.md"

# Compatibility requirements (REQUIRED)
requires:
  # Spec-kit version (semantic version range)
  pdca_version: ">=0.1.0,<2.0.0"
  ...
  # Core pdca-kit commands this extension depends on
  commands:
    - "pdca.tasks"             # Extension needs tasks command
...
hooks:
  # Hook fired after /pdca.tasks completes
  after_tasks:
  ...
  # Hook fired after /pdca.implement completes
  after_implement:
    command: "pdca.jira.sync-status"
    optional: true
...
changelog: "https://github.com/statsperform/pdca-kit-jira/blob/main/CHANGELOG.md"

# Support information (OPTIONAL)
support:
  documentation: "https://github.com/statsperform/pdca-kit-jira/blob/main/docs/"
  issues: "https://github.com/statsperform/pdca-kit-jira/issues"
  discussions: "https://github.com/statsperform/pdca-kit-jira/discussions"
...
4. **Extract**: Unpack to `.pdca/extensions/jira/`
5. **Configure**: Copy config templates
6. **Register**: Add commands to AI agent config
7. **Record**: Update `.pdca/extensions/.registry`

**Registry Format** (`.pdca/extensions/.registry`):
...
1. AI agent finds command in `.claude/commands/pdca.jira.specstoissues.md`
...
3. Remove from `.pdca/extensions/jira/`
4. Update registry
...
**Output**: `.claude/commands/pdca.jira.specstoissues.md`
...
tools:
  ...
  - 'jira-mcp-server/story_create'
scripts:
  sh: .pdca/scripts/bash/check-prerequisites.sh --json
  ps: .pdca/scripts/powershell/check-prerequisites.ps1 -Json
---
...
## Steps
1. Load jira-config.yml from .pdca/extensions/jira/
2. Parse spec.md and tasks.md
3. Create Jira items
```
...
**Output**: `.gemini/commands/pdca.jira.specstoissues.toml`
...
**Location**: `src/pdca_cli/extensions.py`
...
def register_extension_commands(
    ...
    ext_dir = project_path / ".pdca" / "extensions" / ext_id
...
```yaml
# .pdca/extensions/jira/jira-config.yml (Project config)
project:
  key: "MSATS"
...
```yaml
# .pdca/extensions/jira/jira-config.local.yml (Local overrides - gitignored)
project:
...
```bash
config_json=$(bash .pdca/extensions/jira/scripts/parse-jira-config.sh)
echo "$config_json"
```
...
**Script**: `.pdca/extensions/jira/scripts/parse-jira-config.sh`

```bash
#!/usr/bin/env bash
set -euo pipefail

EXT_DIR=".pdca/extensions/jira"
...
if [ -n "${PDCA_JIRA_PROJECT_KEY:-}" ]; then
  defaults=$(echo "$defaults" | jq ".project.key = \"$PDCA_JIRA_PROJECT_KEY\"")
fi
...
schema = load_yaml(".pdca/extensions/jira/extension.yml")['config_schema']
...
**File**: `.pdca/extensions.yml` (project-level extension config)
...
if [ -f ".pdca/extensions.yml" ]; then
  # Parse hooks for after_tasks
  hooks=$(yq eval '.hooks.after_tasks[] | select(.enabled == true)' .pdca/extensions.yml -o=json)
...
The AI agent sees `EXECUTE_COMMAND: pdca.jira.specstoissues` in output and automatically invokes that command.
...
PDCA Kit uses two catalog files with different purposes:
...
1. **Discover**: `pdca extension search` searches both catalogs — community extensions appear automatically
2. **Review**: Evaluate community extensions for security, quality, and organizational fit
3. **Curate**: Copy approved entries from community catalog to your `catalog.json`, or add to `.pdca/extension-catalogs.yml` with `install_allowed: true`
4. **Install**: Use `pdca extension add <name>` — only allowed from `install_allowed: true` catalogs
...
```json
{
  ...
  "extensions": {
    "jira": {
      ...
      "version": "1.0.0",
      "download_url": "https://github.com/statsperform/pdca-kit-jira/releases/download/v1.0.0/pdca-kit-jira-1.0.0.zip",
      "repository": "https://github.com/statsperform/pdca-kit-jira",
      "homepage": "https://github.com/statsperform/pdca-kit-jira/blob/main/README.md",
      "documentation": "https://github.com/statsperform/pdca-kit-jira/blob/main/docs/",
      "changelog": "https://github.com/statsperform/pdca-kit-jira/blob/main/CHANGELOG.md",
    ...
    "linear": {
      ...
      "id": "linear",
      "description": "Sync pdca-kit tasks with Linear issues",
      "author": "Community",
      "version": "0.9.0",
      "download_url": "https://github.com/example/pdca-kit-linear/releases/download/v0.9.0/pdca-kit-linear-0.9.0.zip",
      "repository": "https://github.com/example/pdca-kit-linear",
      "requires": {
        "pdca_version": ">=0.1.0"
      },
...
PDCA Kit supports a **catalog stack** — an ordered list of catalogs that the CLI merges and searches across. This allows organizations to maintain their own org-approved extensions alongside an internal catalog and community discovery, all at once.

#### Catalog Stack Resolution

The active catalog stack is resolved in this order (first match wins):

1. **`PDCA_CATALOG_URL` environment variable** — single catalog replacing all defaults (backward compat)
2. **Project-level `.pdca/extension-catalogs.yml`** — full control for the project
3. **User-level `~/.pdca/extension-catalogs.yml`** — personal defaults
...
This means `pdca extension search` surfaces community extensions out of the box, while `pdca extension add` is still restricted to entries from catalogs with `install_allowed: true`.

#### `.pdca/extension-catalogs.yml` Config File
...
A user-level equivalent lives at `~/.pdca/extension-catalogs.yml`. When a project-level config is present with one or more catalog entries, it takes full control and the built-in defaults are not applied. An empty `catalogs: []` list is treated the same as no config file, falling back to defaults.
...
pdca extension catalog add --name "internal" --install-allowed \
  https://internal.company.com/pdca-kit/catalog.json

# Add a discovery-only catalog
pdca extension catalog add --name "community" \
  https://raw.githubusercontent.com/github/pdca-kit/main/extensions/catalog.community.json
...
Extensions from discovery-only catalogs are shown in `pdca extension search` results but cannot be installed directly:

```
⚠  'linear' is available in the 'community' catalog but installation is not allowed from that catalog.

To enable installation, add 'linear' to an approved catalog (install_allowed: true) in .pdca/extension-catalogs.yml.
```

#### `PDCA_CATALOG_URL` (Backward Compatibility)

The `PDCA_CATALOG_URL` environment variable still works — it is treated as a single `install_allowed: true` catalog, **replacing both defaults** for full backward compatibility:

```bash
# Point to your organization's catalog
export PDCA_CATALOG_URL="https://internal.company.com/pdca-kit/catalog.json"

# All extension commands now use your custom catalog
pdca extension search       # Uses custom catalog
pdca extension add jira     # Installs from custom catalog
```
...
Installed Extensions:
  ✓ Jira Integration (v1.0.0)
     jira
     Create Jira issues from pdca-kit artifacts
     Commands: 3 | Hooks: 2 | Priority: 10 | Status: Enabled

  ✓ Linear Integration (v0.9.0)
     linear
     Create Linear issues from pdca-kit artifacts
...
│ Jira Integration                                        │
│                                                         │
│ Create Jira Epics, Stories, and Issues from pdca-kit   │
│ artifacts                                               │
│                                                         │
│ Author: Stats Perform                                   │
...
│ Downloads: 1,250                                        │
│                                                         │
│ Repository: github.com/statsperform/pdca-kit-jira       │
│ Documentation: github.com/.../docs                      │
└─────────────────────────────────────────────────────────┘

Install: pdca extension add jira
```
...
Requirements:
  • PDCA Kit: >=0.1.0,<2.0.0
  • Tools: jira-mcp-server (>=1.0.0)

Provides:
  Commands:
    • pdca.jira.specstoissues - Create Jira hierarchy from spec and tasks
    • pdca.jira.discover-fields - Discover Jira custom fields
    • pdca.jira.sync-status - Sync task completion status
...
✓ Downloaded pdca-kit-jira-1.0.0.zip (245 KB)
✓ Validated manifest
✓ Checked compatibility (pdca-kit 0.1.0 ≥ 0.1.0)
✓ Extracted to .pdca/extensions/jira/
✓ Registered 3 commands with claude
✓ Installed config template (jira-config.yml)

⚠  Configuration required:
   Edit .pdca/extensions/jira/jira-config.yml to set your Jira project key

Extension installed successfully!

Next steps:
  1. Configure: vim .pdca/extensions/jira/jira-config.yml
  2. Discover fields: /pdca.jira.discover-fields
  3. Use commands: /pdca.jira.specstoissues
```
...
⚠  This will remove:
   ...
   • Extension directory: .pdca/extensions/jira/
   • Config file: jira-config.yml (will be backed up)

Continue? (yes/no): yes

✓ Unregistered commands
✓ Backed up config to .pdca/extensions/.backup/jira-config.yml
✓ Removed extension directory
✓ Updated registry

Extension removed successfully.

To reinstall: pdca extension add jira
```
...
✓ Downloaded pdca-kit-jira-1.1.0.zip
✓ Validated manifest
...
Changelog: https://github.com/statsperform/pdca-kit-jira/blob/main/CHANGELOG.md#v110
```
...
#### `pdca extension enable/disable NAME`
...
```bash
$ pdca extension set-priority jira 5
...
def check_compatibility(extension_manifest: dict) -> bool:
    ...
    # 1. Check pdca-kit version
    current_pdca = get_pdca_version()  # e.g., "0.1.5"
    required_pdca = requires['pdca_version']  # e.g., ">=0.1.0,<2.0.0"

    if not version_satisfies(current_pdca, required_pdca):
        raise IncompatibleVersionError(
            f"Extension requires pdca-kit {required_pdca}, "
            f"but {current_pdca} is installed. "
            f"Upgrade pdca-kit with: uv tool install pdca-cli --force"
        )
    ...
    for cmd in requires.get('commands', []):
        if not command_exists(cmd):
            raise MissingCommandError(
                ...
                f"Update pdca-kit to latest version"
            )
...
provides:
  commands:
    - name: "pdca.jira.old-command"
      file: "commands/old-command.md"
      deprecated: true
      deprecated_message: "Use pdca.jira.new-command instead"
...
```text
⚠️  Warning: /pdca.jira.old-command is deprecated
   Use /pdca.jira.new-command instead
...
- Code reviewed by pdca-kit maintainers
...
permissions:
  - "filesystem:read:.pdca/extensions/jira/"  # Can only read own config
  - "filesystem:write:.pdca/memory/"          # Can write to memory
  - "network:external:*.atlassian.net"           # Can call Jira API
  - "env:read:PDCA_JIRA_*"                    # Can read own env vars
```
...
"jira": {
  ...
  "signature": "https://github.com/.../pdca-kit-jira-1.0.0.sig",
...
**Goal**: Existing pdca-kit projects work without changes.

**Strategy**:

1. **Core commands unchanged**: `/pdca.tasks`, `/pdca.implement`, etc. remain in core
...
pdca extension add github-projects
/pdca.github.taskstoissues
```
...
provides:
  commands:
    - name: "pdca.github.taskstoissues"
      file: "commands/taskstoissues.md"
      aliases: ["pdca.github.sync-taskstoissues"]  # Alternate namespaced entry point
```

AI agents register both names, so callers can migrate to the alternate alias without relying on deprecated global shortcuts like `/pdca.taskstoissues`.
...
- [x] CLI commands:
  - [x] `pdca extension list`
  - [x] `pdca extension add` (from URL and local `--dev`)
  - [x] `pdca extension remove`
- [x] Extension registry (`.pdca/extensions/.registry`)
...
- [x] Create `pdca-kit-jira` repository
...
- [x] Central catalog (`extensions/catalog.json` in pdca-kit repo)
- [x] Community catalog (`extensions/catalog.community.json`)
- [x] Catalog fetch and parsing with multi-catalog support
- [x] CLI commands:
  - [x] `pdca extension search`
  - [x] `pdca extension info`
  - [x] `pdca extension catalog list`
  - [x] `pdca extension catalog add`
  - [x] `pdca extension catalog remove`
...
- [x] Project extensions config (`.pdca/extensions.yml`)
- [x] CLI commands:
  - [x] `pdca extension update` (with atomic backup/restore)
  - [x] `pdca extension enable/disable`
...
- [x] **Cache invalidation**: Cache invalidated when `PDCA_CATALOG_URL` changes
...
**Decision**: **Option C** - Both prefixed and aliases are supported. Commands use `pdca.{extension}.{command}` as canonical name, with optional aliases defined in manifest.
...
**Decision**: **Option A** - Extension directory (`.pdca/extensions/{ext-id}/{ext-id}-config.yml`). This keeps extensions self-contained and easier to manage.
...
**Decision**: **Option A** - Hooks are registered in `.pdca/extensions.yml` and executed by the AI agent when it sees the hook trigger. Hook state (enabled/disabled) is managed per-extension.
...
**Complete structure of `pdca-kit-jira` extension:**
...
| Extension Feature | PDCA Kit Version | AI Agent Support |
|-------------------|------------------|------------------|
...
This RFC proposes a comprehensive extension system for PDCA Kit that:
