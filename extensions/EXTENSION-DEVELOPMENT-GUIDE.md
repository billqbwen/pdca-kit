...
A guide for creating PDCA Kit extensions.
...
extension:
  ...
  author: "Your Name"
  repository: "https://github.com/you/pdca-kit-my-ext"
  license: "MIT"

requires:
  pdca_version: ">=0.1.0"            # Minimum pdca-kit version
...
provides:
  commands:
    - name: "pdca.my-ext.hello"      # Must follow pattern: pdca.{ext-id}.{cmd}
      file: "commands/hello.md"
      description: "Say hello"
      aliases: ["pdca.my-ext.hi"]    # Optional aliases, same pattern
...
Load extension config from `.pdca/extensions/my-ext/my-ext-config.yml`.

### 5. Test Locally

```bash
cd /path/to/pdca-kit-project
pdca extension add --dev /path/to/my-extension
```
...
```bash
claude
> /pdca.my-ext.hello world
```

The command will be available in `.claude/commands/pdca.my-ext.hello.md`.
...
- `pdca_version`: Semantic version specifier (e.g., ">=0.1.0,<2.0.0")

**Optional sub-fields**:

- `tools`: External tools required (array of tool objects)
- `commands`: Core pdca-kit commands needed (array of command names)
...
- `name`: Command name (must match `pdca.{ext-id}.{command}`)
- `file`: Path to command file (relative to extension root)
- `description`: Command description (optional)
- `aliases`: Alternative command names (optional, array; each must match `pdca.{ext-id}.{command}`)
...
```yaml
scripts:
  sh: .pdca/scripts/bash/helper.sh
```

This allows scripts to reference core pdca-kit scripts.
...
2. Project config (`.pdca/extensions/my-ext/my-ext-config.yml`)
3. Local overrides (`.pdca/extensions/my-ext/my-ext-config.local.yml` - gitignored)
4. Environment variables (`PDCA_MY_EXT_*`)

**Example loading script**:

```bash
#!/usr/bin/env bash
EXT_DIR=".pdca/extensions/my-ext"

# Load and merge config
config=$(yq eval '.' "$EXT_DIR/my-ext-config.yml" -o=json)

# Apply env overrides
if [ -n "${PDCA_MY_EXT_API_KEY:-}" ]; then
  config=$(echo "$config" | jq ".api.api_key = \"$PDCA_MY_EXT_API_KEY\"")
fi
...
Extension authors can create a `.extensionignore` file in the extension root to exclude files and folders from being copied when a user installs the extension with `pdca extension add`. This is useful for keeping development-only files (tests, CI configs, docs source, etc.) out of the installed copy.
...
- **Pattern**: `^pdca\.[a-z0-9-]+\.[a-z0-9-]+$`
- **Valid**: `pdca.my-ext.hello`, `pdca.tool.cmd`
- **Invalid**: `my-ext.hello` (missing prefix), `pdca.hello` (no extension namespace)
...
5. **Check command registration**:
   ...
   ```bash
   ls .claude/commands/pdca.my-ext.*
   ```

6. **Remove extension**:

   ```bash
   pdca extension remove my-ext
   ```
...
1. **Create repository**: `pdca-kit-my-ext`
2. **Add files**:
...
4. **Install from repo**:
   ...
   ```bash
   git clone https://github.com/you/pdca-kit-my-ext
   pdca extension add --dev pdca-kit-my-ext/
   ```
...
```bash
zip -r pdca-kit-my-ext-1.0.0.zip extension.yml commands/ scripts/ docs/
```

Users install with:

```bash
pdca extension add <extension-name> --from https://github.com/.../pdca-kit-my-ext-1.0.0.zip
```
...
2. **File an issue** using the [Extension Submission](https://github.com/github/pdca-kit/issues/new?template=extension_submission.yml) template
3. **After review**, a maintainer updates the catalog and your extension becomes available:
   ...
   - Users install with: `pdca extension add my-ext` (from their catalog)
...
Load config:
```bash
config_file=".pdca/extensions/tool/tool-config.yml"
...
**Error**: `Extension requires pdca-kit >=0.2.0`

- **Fix**: Update pdca-kit with `uv tool install pdca-cli --force --from git+https://github.com/github/pdca-kit.git`. The bare `pdca-cli` package on PyPI is a different, unrelated project — installing it without `--from git+...` will give you a stub CLI that does not include `extension`, `preset`, or other pdca-kit commands.
...
1. Config file exists: `.pdca/extensions/{ext-id}/{ext-id}-config.yml`
...
- **Examples**: See `pdca-kit-jira` for full-featured example (Phase B)
