...
A custom preset for PDCA Kit. Copy this directory and customize it to create your own.
...
| `pdca.define` | command | Custom specification command (overrides core) |
| `pdca.myext.myextcmd` | command | Override of the myext extension's myextcmd command |
...
4. Test locally: `pdca preset add --dev ./my-preset`
5. Verify resolution: `pdca preset resolve spec-template`
6. Remove when done testing: `pdca preset remove my-preset`
...
- `requires.pdca_version` — version constraint (e.g. `>=0.1.0`)
...
- **command** — AI agent workflow prompts (e.g. pdca.define, pdca.plan)
